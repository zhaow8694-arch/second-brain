...
//+------------------------------------------------------------------+
#property copyright "Asymmetric System - Apex V3"
...
input double   InpRiskPercent   = 5.0;       // 单次总风险 5% (将拆分给双子星)
input int      InpAtrPeriod     = 14;        
input double   InpSlMultiplier  = 0.8;       // 极窄初始止损 (极易扫损，换取大仓位)
...
int handle_atr;
double atr_buffer[];
double base_lot_size = 0; // 记录基础单量，加仓时保持一致
   ...
   if(my_positions == 0)
     {
      ...
      base_lot_size = NormalizeDouble(risk_money_per_trade / (sl_distance / tick_size * tick_value), 2);
            ...
            if(bid < last_entry_sell - pyramid_distance)
              {
               ...
               Print("?? 狂暴加仓(空)！当前加仓层级: ", addon_count + 1);
               last_entry_sell = bid; 
              }
           }
        }
