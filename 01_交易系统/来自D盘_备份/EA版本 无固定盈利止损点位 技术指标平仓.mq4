...
input int MaxSameDirectionOrders = 8;      // 同方向最大持仓数 (普通仓位2个 + 应急仓位2个 + 智能加仓4个)
...
// 锁仓管理 - 统一触发模式（已禁用）
input bool EnableLockManagement = false;   // 启用锁仓管理（已禁用）
...
// 建仓频率控制参数
input int MinOrderInterval = 0;            // 暂时取消建仓间隔限制(原15分钟=900秒)
input int MaxNormalOrders = 2;             // 普通仓最多2个持仓
input int MaxLockOrders = 0;               // 锁仓单最多0个持仓（已禁用）
...
// 锁仓单亏损平仓参数
input double LockOrderLossLimit = 9000.0;       // 锁仓单亏损平仓点位（从4000点提高到9000点）

// 决策评分反转处理参数
input bool EnableDecisionScoreReversal = true;     // 启用决策评分反转处理(普通仓位扛单模式)
// input double SmallLossThreshold = 500.0;        // 小亏损阈值 - 已废弃
// input double LargeLossThreshold = 2000.0;       // 大亏损阈值 - 已废弃
input int DecisionReversalTicks = 3;              // 决策反转确认tick数
input double DecisionReversalThreshold = 0.75;    // 决策反转概率阈值（概率制：75%）
// input int BatchCloseInterval = 10;               // 分批平仓间隔(tick数) - 已废弃

// 动态止盈止损参数
input bool EnableDynamicTP = true;                 // 启用动态止盈止损
input double DefaultATRMultiplier = 2.0;          // 默认ATR倍数（止损）
input double DefaultTPMultiplier = 3.0;           // 默认ATR倍数（止盈）
input bool EnablePartialTakeProfit = true;        // 启用分段止盈
input double PartialCloseRatio = 0.3;             // 分段平仓比例（30%）
input int PartialCloseLevels = 3;                 // 分段平仓级别数

// 应急仓位参数
input int EmergencyOrderCount = 2;               // 应急仓位数量（调整为2个）
input double EmergencyLotSize = 0.02;            // 应急仓位手数
input double EmergencyProfitTarget = 250.0;      // 应急仓位盈利目标(点位)
input double EmergencyStopLoss = 9000.0;          // 应急仓位止损(点位)
...
input double SmartBuyDecisionThreshold = 0.70;     // 智能加仓决策评分阈值（概率制：70%以上）
input int MaxSmartBuyOrders = 4;              // 智能加仓最大订单数（调整为4个）
...
input double SmartBuyStopLoss = 9000.0;        // 智能加仓止损（点数）
...
{
private:
    static double g_cached_atr;
    static datetime g_last_atr_update;
    static int g_atr_period;
    
    static double CalculateATR(int period)
    {
        // 检查缓存是否有效（每5秒更新一次）
        if(g_last_atr_update > 0 && TimeCurrent() - g_last_atr_update < 5 && g_atr_period == period)
        {
            return g_cached_atr;
        }
        
        double atr = 0.0;
        int bars_available = Bars;
        
        // 确保有足够的数据
        if(bars_available < period + 1)
        {
            Print("⚠️ ATR计算警告：可用K线数量不足，使用可用数据计算");
            period = bars_available - 1;
        }
        
        for(int i = 1; i <= period; i++)
        {
            if(i < bars_available && i + 1 < bars_available)
            {
...
public:
    // 计算动态止损点数（基于ATR）
    static double CalculateDynamicStopLoss(int order_type, double atr_multiplier = 0.0)
    {
        if(!EnableDynamicTP) return EmergencyStopLoss; // 如果禁用动态止盈止损，使用固定值
        
        if(atr_multiplier == 0.0) atr_multiplier = DefaultATRMultiplier;
        double atr = CalculateATR(14); // 使用14周期ATR
        double atr_pips = atr / Point;
        return atr_pips * atr_multiplier;
    }
    
    // 计算动态止盈点数（基于ATR）
    static double CalculateDynamicTakeProfit(int order_type, double atr_multiplier = 0.0)
    {
        if(!EnableDynamicTP) return EmergencyProfitTarget; // 如果禁用动态止盈止损，使用固定值
        
        if(atr_multiplier == 0.0) atr_multiplier = DefaultTPMultiplier;
        double atr = CalculateATR(14); // 使用14周期ATR
        double atr_pips = atr / Point;
        return atr_pips * atr_multiplier;
    }
    ...
    // 计算分段止盈点数
    static double CalculatePartialTakeProfit(int order_type, int partial_level, double atr_multiplier = 0.0)
    {
        if(!EnablePartialTakeProfit) return 0.0; // 如果禁用分段止盈，返回0
        
        if(atr_multiplier == 0.0) atr_multiplier = DefaultTPMultiplier * 0.5; // 分段止盈使用较小的倍数
        double atr = CalculateATR(14);
        double atr_pips = atr / Point;
        return atr_pips * atr_multiplier * partial_level;
    }
};

// ATR动态止盈止损类静态成员变量定义
double CATRDynamicTP::g_cached_atr = 0.0;
datetime CATRDynamicTP::g_last_atr_update = 0;
int CATRDynamicTP::g_atr_period = 0;

//+------------------------------------------------------------------+
//| 分段止盈管理类
//+------------------------------------------------------------------+
class CPartialTakeProfit
{
private:
    struct PartialData
    {
        int ticket;
        int partial_level;        // 当前分段级别 (1, 2, 3)
        double partial_lots;      // 分段平仓手数
        double total_lots;        // 总手数
        datetime last_partial_time;
    };
    
    static PartialData partial_records[100];
...
public:
    static void Init()
    {
        ...
        for(int i = 0; i < 100; i++)
        {
            partial_records[i].ticket = 0;
            partial_records[i].partial_level = 0;
            partial_records[i].partial_lots = 0;
            partial_records[i].total_lots = 0;
            partial_records[i].last_partial_time = 0;
        }
    }
    
    // 检查分段止盈条件
    static bool CheckPartialTakeProfit(int ticket)
    {
        if(!OrderSelect(ticket, SELECT_BY_TICKET)) return false;
        
        // 查找记录
        int record_index = FindRecord(ticket);
        ...
        double partial_target = CATRDynamicTP::CalculatePartialTakeProfit(OrderType(), partial_records[record_index].partial_level);
        
        // 检查是否达到分段止盈目标
        if(profit_pips >= partial_target)
        {
            // 检查是否超过最大分段级别
            if(partial_records[record_index].partial_level >= PartialCloseLevels)
            {
                Print("💰 分段止盈达到最大级别: 订单=", ticket, " 级别=", partial_records[record_index].partial_level);
                return false;
            }
            
            // 计算分段平仓手数
            double remaining_lots = partial_records[record_index].total_lots - partial_records[record_index].partial_lots;
            double partial_close_lots = MathMin(remaining_lots * PartialCloseRatio, remaining_lots); // 使用配置的平仓比例
            
            // 确保最小平仓手数
            if(partial_close_lots < 0.01)
            {
                partial_close_lots = 0.01; // 最小平仓0.01手
            }
            
            if(partial_close_lots > 0)
            {
                // 执行分段平仓
                if(OrderClose(ticket, partial_close_lots, (OrderType() == OP_BUY) ? Bid : Ask, 3, clrYellow))
                {
                    partial_records[record_index].partial_lots += partial_close_lots;
                    partial_records[record_index].partial_level++;
                    partial_records[record_index].last_partial_time = TimeCurrent();
                    
                    Print("💰 分段止盈成功: 订单=", ticket, " 级别=", partial_records[record_index].partial_level, 
                          " 平仓手数=", partial_close_lots, " 目标=", partial_target, "点");
                    
                    // 如果全部平仓，移除记录
                    if(partial_records[record_index].partial_lots >= partial_records[record_index].total_lots)
                    {
                ...
                else
                {
                    ...
                    Print("❌ 分段止盈失败: 订单=", ticket, " 错误=", error);
                }
            }
        }
    ...
    // 添加新订单到分段止盈管理
    static void AddOrder(int ticket, double lots)
    {
        if(record_count >= 100) return;
        
        partial_records[record_count].ticket = ticket;
        partial_records[record_count].partial_level = 1;
        partial_records[record_count].partial_lots = 0;
        partial_records[record_count].total_lots = lots;
        partial_records[record_count].last_partial_time = TimeCurrent();
        
        record_count++;
    }
    
private:
    static int FindRecord(int ticket)
    {
        for(int i = 0; i < record_count; i++)
        {
            if(partial_records[i].ticket == ticket) return i;
        }
        return -1;
    }
    ...
    static void RemoveRecord(int index)
    {
        ...
        {
            partial_records[i] = partial_records[i + 1];
        }
        record_count--;
    }
};
...
// 静态成员变量定义
PartialData CPartialTakeProfit::partial_records[100];
int CPartialTakeProfit::record_count = 0;

// 市场状态常量定义（全局常量，用于switch语句）
#define REGIME_TRENDING 0      // 趋势市场
#define REGIME_RANGING 1       // 震荡市场
#define REGIME_BREAKOUT 2      // 突破市场
#define REGIME_REVERSAL 3      // 反转市场

// 注意：静态成员变量定义将在类定义之后进行
...
{
private:
    static double g_cached_trend_strength;
    static double g_cached_volatility;
    static datetime g_last_market_update;
    
    static double CalculateTrendStrength(int period = 20)
    {
        // 检查缓存是否有效（每10秒更新一次）
        if(g_last_market_update > 0 && TimeCurrent() - g_last_market_update < 10)
        {
            return g_cached_trend_strength;
        }
        
        int bars_available = Bars;
        if(bars_available < period * 2)
        {
            Print("⚠️ 趋势强度计算警告：可用K线数量不足");
            return 0.0;
        }
        ...
        {
            if(i < bars_available && i + period < bars_available)
            {
                ma_fast += Close[i];
                ma_slow += Close[i + period];
            }
        }
        ma_fast /= period;
        ma_slow /= period;
        
        g_cached_trend_strength = MathAbs(ma_fast - ma_slow) / Point;
        return g_cached_trend_strength;
    }
    ...
    static double CalculateVolatility(int period = 14)
    {
        ...
        if(g_last_market_update > 0 && TimeCurrent() - g_last_market_update < 10)
        {
            return g_cached_volatility;
        }
        
        int bars_available = Bars;
        if(bars_available < period + 1)
        {
            Print("⚠️ 波动率计算警告：可用K线数量不足");
            return 0.0;
        }
        ...
        g_cached_volatility = volatility / period;
        g_last_market_update = TimeCurrent();
        return g_cached_volatility;
    }
    
public:
    // 使用整数常量替代枚举，提高MQL4兼容性
    // 注意：这些常量将在类外部定义为全局常量
    
    static int DetectMarketRegime()
    {
        double trend_strength = CalculateTrendStrength();
        double volatility = CalculateVolatility();
        double atr = CATRDynamicTP::CalculateDynamicStopLoss(OP_BUY, 1.0) * Point;
        
        // 趋势市场：趋势强度高，波动率中等
        if(trend_strength > 100 && volatility > atr * 0.5 && volatility < atr * 2.0)
        {
            return REGIME_TRENDING;
        }
        // 震荡市场：趋势强度低，波动率中等
        else if(trend_strength < 50 && volatility > atr * 0.3 && volatility < atr * 1.5)
        {
            return REGIME_RANGING;
        }
        // 突破市场：趋势强度高，波动率高
        else if(trend_strength > 150 && volatility > atr * 2.0)
        {
    ...
    // 根据市场状态调整ATR倍数
    static double GetATRMultiplier(int regime, bool is_stop_loss)
    {
        if(is_stop_loss)
        {
            switch(regime)
            {
                case REGIME_TRENDING: return 2.5;      // 趋势市场：止损宽松
                case REGIME_RANGING: return 1.5;       // 震荡市场：止损收紧
                case REGIME_BREAKOUT: return 3.0;      // 突破市场：止损宽松
                case REGIME_REVERSAL: return 1.0;      // 反转市场：止损收紧
                default: return 2.0;
            }
        }
        else
        {
            switch(regime)
            {
                case REGIME_TRENDING: return 4.0;      // 趋势市场：止盈宽松
                case REGIME_RANGING: return 2.5;       // 震荡市场：止盈收紧
                case REGIME_BREAKOUT: return 5.0;      // 突破市场：止盈宽松
                case REGIME_REVERSAL: return 2.0;      // 反转市场：止盈收紧
                default: return 3.0;
            }
        }
    }
...
// 市场状态检测类静态成员变量定义
double CMarketRegimeDetector::g_cached_trend_strength = 0.0;
double CMarketRegimeDetector::g_cached_volatility = 0.0;
datetime CMarketRegimeDetector::g_last_market_update = 0;
...
public:
    ...
    void ExecuteLockOrder(int order_type, string comment)
    {
        ...
        if(order_type == OP_BUY)
        {
            stop_loss = price - LockOrderLossLimit * Point;    // 止损9000点
...
public:
    ...
    // 新增：计算普通仓位的主要方向
    int GetNormalOrderMainDirection()
    {
        int buy_count = 0;
        int sell_count = 0;
        ...
        Print("📊 普通仓位方向统计: 买入=", buy_count, " 卖出=", sell_count);
    ...
    void ExecuteEmergencyOrder(int order_type, double buy_score, double sell_score)
    {
        ...
        if(emergency_order_type == OP_BUY)
        {
            ...
            if(ticket > 0)
            {
                ...
                // 添加到分段止盈管理
                CPartialTakeProfit::AddOrder(ticket, EmergencyLotSize);
                Print("💰 应急仓位已添加到分段止盈管理");
    ...
    void ExecuteTradingLogic()
    {
        ...
        if(::CountNormalOrders() >= MaxNormalOrders)
        {
            ...
            if(g_current_reversal_signal.has_reversal && g_current_reversal_signal.is_valid)
            {
                ...
                Print("📊 反转信号强度: ", g_current_reversal_signal.confidence, " 阈值: ", DecisionReversalThreshold);
                
                // 🚨 修复：应急仓位方向基于普通仓对冲，而不是反转信号方向
                int normal_main_direction = ai_predictor.GetNormalOrderMainDirection();
                int emergency_direction = -1; // 默认无方向
                
                if(normal_main_direction == OP_BUY)
                {
                    emergency_direction = OP_SELL; // 普通仓买入 → 应急仓卖出
                    Print("📊 应急仓位对冲逻辑: 普通仓买入 → 应急仓卖出");
                }
                else if(normal_main_direction == OP_SELL)
                {
                    emergency_direction = OP_BUY; // 普通仓卖出 → 应急仓买入
                    Print("📊 应急仓位对冲逻辑: 普通仓卖出 → 应急仓买入");
                }
                else
                {
                    Print("📊 应急仓位触发失败: 普通仓位无明确主要方向，无法确定对冲方向");
                    return; // 无法确定对冲方向，跳过
                }
                
                Print("📊 应急仓位方向: ", emergency_direction == OP_BUY ? "买入" : "卖出", " (基于普通仓对冲)");
...
private:
    int MakeFinalDecision(int ai_prediction, double ai_confidence, bool has_ai_signal, bool market_suitable, double buy_score, double sell_score)
    {
        ...
        // 最终决策 - 优化阈值平衡交易频率和质量
        double decision_threshold = 0.65; // 概率制：需要65%以上概率才交易
    ...
    void ExecuteOrder(int order_type)
    {
        ...
        if(order_type == OP_BUY)
        {
            ...
            if(ticket > 0)
            {
                ...
                // 添加到分段止盈管理
                CPartialTakeProfit::AddOrder(ticket, lot_size);
                Print("💰 普通订单已添加到分段止盈管理");
        ...
        else if(order_type == OP_SELL)
        {
            ...
            if(sell_ticket > 0)
            {
                ...
                // 添加到分段止盈管理
                CPartialTakeProfit::AddOrder(sell_ticket, lot_size);
                Print("💰 普通订单已添加到分段止盈管理");
        ...
        for(int idx2 = OrdersTotal() - 1; idx2 >= 0; idx2--)
        {
            if(OrderSelect(idx2, SELECT_BY_POS, MODE_TRADES))
            {
                if(IsOurOrder())
                {
                    ...
                    // 0. 动态止盈平仓（最高优先级）- 智能加仓订单和应急仓位除外
                    if(!is_smart_buy && !is_emergency) // 智能加仓订单和应急仓位跳过通用止盈平仓
                    {
                        // 修复：点位计算（current_profit已经包含手数信息）
                        double profit_pips = current_profit / Point; // 正确计算点位
                        
                        // 检测市场状态，动态调整止盈目标
                        int regime = CMarketRegimeDetector::DetectMarketRegime();
                        double atr_multiplier = CMarketRegimeDetector::GetATRMultiplier(regime, false);
                        double dynamic_tp = CATRDynamicTP::CalculateDynamicTakeProfit(OrderType(), atr_multiplier);
                        
                        Print("📊 订单 ", OrderTicket(), " 盈利检查: 美元=", current_profit, " 点位=", profit_pips, " 手数=", OrderLots());
                        Print("🌍 市场状态: ", (regime == 0 ? "趋势" : regime == 1 ? "震荡" : regime == 2 ? "突破" : "反转"), " ATR倍数: ", atr_multiplier, " 动态止盈: ", dynamic_tp, "点");
                        
                        if(profit_pips >= dynamic_tp)
                        {
                            Print("🎯 订单 ", OrderTicket(), " 达到动态止盈目标 ", dynamic_tp, " 点，执行平仓");
                            CloseOrder(OrderTicket(), "动态止盈平仓");
                            continue;
                        }
                        
                        // 检查分段止盈
                        if(CPartialTakeProfit::CheckPartialTakeProfit(OrderTicket())) {
                            continue;
                        }
                    }
...
private:
    ...
    bool CheckBatchCloseCondition(int ticket, double reversal_confidence)
    {
        ...
        // 检查是否到了分批平仓时间
        if(current_time - last_batch_close_time >= 10) // 使用固定值10秒，替代已废弃的BatchCloseInterval
        {
            // 根据置信度决定平仓概率
    ...
    bool CheckEmergencyOrderClose(int ticket)
    {
        ...
        // 检测市场状态，动态调整止盈止损目标
        int regime = CMarketRegimeDetector::DetectMarketRegime();
        double tp_multiplier = CMarketRegimeDetector::GetATRMultiplier(regime, false);
        double sl_multiplier = CMarketRegimeDetector::GetATRMultiplier(regime, true);
        
        double dynamic_tp = CATRDynamicTP::CalculateDynamicTakeProfit(OrderType(), tp_multiplier);
        double dynamic_sl = CATRDynamicTP::CalculateDynamicStopLoss(OrderType(), sl_multiplier);
        
        Print("🌍 应急仓位市场状态: ", (regime == 0 ? "趋势" : regime == 1 ? "震荡" : regime == 2 ? "突破" : "反转"), 
              " 止盈倍数: ", tp_multiplier, " 止损倍数: ", sl_multiplier);
        Print("📊 应急仓位动态目标: 止盈=", dynamic_tp, "点 止损=", dynamic_sl, "点");
        
        // 手动执行平仓逻辑
        if(profit_pips >= dynamic_tp)
        {
            Print("🎯 应急仓位达到动态盈利目标: 订单=", ticket, " 盈利=", profit_pips, "点 目标=", dynamic_tp, "点");
            CloseOrder(ticket, "应急仓位动态盈利目标平仓");
            return true;
        }
        else if(profit_pips <= -dynamic_sl)
        {
            Print("🛑 应急仓位触发动态止损: 订单=", ticket, " 亏损=", profit_pips, "点 止损=", dynamic_sl, "点");
            CloseOrder(ticket, "应急仓位动态止损平仓");
            return true;
        }
    ...
    bool CheckSmartBuyOrderClose(int ticket)
    {
        ...
        Print("🌍 智能加仓市场状态: ", (regime == 0 ? "趋势" : regime == 1 ? "震荡" : regime == 2 ? "突破" : "反转"), 
              " 止盈倍数: ", tp_multiplier, " 止损倍数: ", sl_multiplier);
        Print("📊 智能加仓动态目标: 止盈=", dynamic_tp, "点 止损=", dynamic_sl, "点");
        
        // 记录智能加仓订单状态
        Print("📊 智能加仓订单状态检查: 订单=", ticket, " 盈利=", profit_pips, "点 动态目标=", dynamic_tp, "点 动态止损=", dynamic_sl, "点");
        
        // 手动执行平仓逻辑
        if(profit_pips >= dynamic_tp)
        {
            Print("🎯 智能加仓订单达到动态盈利目标: 订单=", ticket, " 盈利=", profit_pips, "点 目标=", dynamic_tp, "点");
            CloseOrder(ticket, "智能加仓动态盈利目标平仓");
            return true;
        }
        else if(profit_pips <= -dynamic_sl)
        {
            Print("🛑 智能加仓订单触发动态止损: 订单=", ticket, " 亏损=", profit_pips, "点 止损=", dynamic_sl, "点");
            CloseOrder(ticket, "智能加仓动态止损平仓");
            return true;
        }
...
public:
    ...
    WeightAllocation GetWeightAllocation(MARKET_REGIME regime)
    {
        ...
        switch(regime) {
            ...
            case TRENDING_DOWN:
                // 趋势市场：适度增强技术指标权重
                weights.technical_weight = 0.72; // 72% (适度增强)
                weights.market_weight = 0.28;    // 28%
                weights.synergy_bonus = 0.1;     // 10%协同奖励
                break;
                
            case RANGING:
                // 震荡市场：保持平衡分配，接近基础权重
                weights.technical_weight = 0.68; // 68% (接近基础70%)
                weights.market_weight = 0.32;    // 32% (接近基础30%)
            ...
            case BREAKOUT_DOWN:
                // 突破市场：适度增强市场状况权重
                weights.technical_weight = 0.65; // 65%
                weights.market_weight = 0.35;    // 35%
            ...
            case REVERSAL_DOWN:
                // 反转市场：适度增强技术指标权重
                weights.technical_weight = 0.74; // 74% (适度增强)
                weights.market_weight = 0.26;    // 26%
...
int OnInit()
{
    ...
    Print("=== AI增强风险管理EA启动 ===");
            Print("版本: 2.8.7 - 应急仓位对冲逻辑修复版 🛡️");
    Print("🔧 关键升级:");
    Print("  🛡️ 应急仓位逻辑修复：确保对冲普通仓");
    Print("  🔍 新增普通仓方向检测功能");
    Print("  ⚡ 应急仓位时机：反转信号强度判断");
    Print("  🎯 应急仓位方向：强制与普通仓相反");
    Print("  📈 持仓上限: ", MaxNormalOrders, "单普通 + ", EmergencyOrderCount, "单应急 + ", MaxSmartBuyOrders, "单智能");
    Print("  ⏰ 建仓频率: 15分钟间隔控制");
    Print("📊 仓位配置优化:");
Print("  🔒 锁仓功能: 已完全禁用");
Print("  🚨 应急仓位: 调整为2个");
Print("  ⚖️ 同方向持仓: 调整为8个");
Print("  📊 决策反转阈值: 从55%提高到75%");
Print("  🛡️ 风险控制: 提高应急仓位触发质量");
    Print("🔧 核心功能:");
    Print("  ✅ 扛单策略: 10000点极限，无时间限制");
            Print("  ✅ 应急仓位: 2个仓位，基于反转信号触发");
    ...
    Print("  🔄 应急仓位管理: 每20个tick执行 (降低计算负担)");
    ...
    Print("锁仓功能: 已禁用");
    Print("=== 高级功能 ===");
    Print("动态止盈止损: ", (EnableDynamicTP ? "启用" : "禁用"));
    Print("分段止盈: ", (EnablePartialTakeProfit ? "启用" : "禁用"));
    if(EnableDynamicTP)
    {
        Print("  默认ATR倍数 - 止损: ", DefaultATRMultiplier, " 止盈: ", DefaultTPMultiplier);
        Print("  分段止盈比例: ", PartialCloseRatio * 100, "% 级别数: ", PartialCloseLevels);
    }
    ...
    Print("同方向最大持仓: ", MaxSameDirectionOrders, " (普通仓位2个 + 应急仓位2个 + 智能加仓4个)");
    Print("=== 手数设置 ===");
    ...
    Print("最大扛单亏损: ", MaxHoldLossPips, " 点");
            ...
            Print("锁仓功能: 已禁用 (原触发点: ", LockTriggerLevel, " 点)");
    Print("锁仓解锁: ", UnlockProfit, "点");
        Print("锁仓单亏损平仓: ", LockOrderLossLimit, "点");
        Print("建仓频率控制: 已暂时取消间隔限制");
    ...
    Print("=== 应急仓位风险控制 ===");
Print("应急仓位止损: ", EmergencyStopLoss, "点");
    
    Print("=== 决策评分反转处理 ===");
    Print("决策评分反转: ", (EnableDecisionScoreReversal ? "启用" : "禁用"));
    Print("🚨 新模式：应急仓位基于反转信号触发，替代锁仓功能");
Print("📊 普通仓位：反转信号后保持持有，不平仓");
Print("🔒 锁仓功能：已完全禁用，应急仓位提供风险控制");
            Print("基础反转阈值: ", DecisionReversalThreshold, " (基于前5个决策评分对比，75%高阈值)");
    ...
    Print("应急止损: ", EmergencyStopLoss, " 点位");
            // Print("应急触发阈值: ", EmergencyTriggerScore, " (已废弃，现在使用DecisionReversalThreshold: ", DecisionReversalThreshold, " - 75%高阈值)");
    Print("🚨 应急仓位只给予反转信号");
...
{
    // === 概率分配式决策评分系统 - 严谨重构版 ===
    // 核心原理：buy_score + sell_score = 1.0 (100%概率分配)
    // 修复：统一强度标准、动态权重分配、完整技术指标覆盖
    ...
    // MACD指标：使用advanced_indicators获取
    double macd_main = advanced_indicators.GetMACDMain();
    ...
    // === 买入vs卖出对抗式概率分配系统 ===
    // 核心逻辑：所有指标分别为买入和卖出提供支持强度
    // 最终概率基于：买入总强度 vs 卖出总强度 的竞争
    
    // 买入/卖出强度累加器
    double total_buy_strength = 0.0;
    double total_sell_strength = 0.0;
    double total_weight = 0.0;
    double active_technical_weight = 0.0; // 实际参与的技术指标权重
    ...
    double base_technical_weight = TechnicalWeight;
    double base_market_weight = MarketWeight;
    ...
    // 获取最终基础权重（如果启用动态调整）
    if(EnableDynamicWeightAdjustment) {
        base_technical_weight = advanced_market_analysis.GetDynamicTechnicalWeight(current_regime);
        base_market_weight = advanced_market_analysis.GetDynamicMarketWeight(current_regime);
    }
    
    Print("📊 基础权重 - 技术:", DoubleToString(base_technical_weight, 2), " 市场:", DoubleToString(base_market_weight, 2));
    
    // === 技术指标买入/卖出强度计算 ===
    
    // 1. EMA趋势信号 (技术权重30%) - 买入vs卖出强度分配
    if(ma_fast != 0 && ma_slow != 0) {
        double ma_diff = ma_fast - ma_slow;
        double ma_diff_pct = MathAbs(ma_diff) / ma_slow * 10000; // 转换为万分比
        double ema_strength = MathMin(ma_diff_pct / 10.0, 1.0); // 10万分点为满强度
        double ema_weight = base_technical_weight * 0.30;
        
        if(ma_fast > ma_slow) {
            // 快线上穿慢线：买入信号
            double ema_buy_contribution = ema_strength * ema_weight;
            total_buy_strength += ema_buy_contribution;
            Print("📊 EMA信号: 买入强度=", DoubleToString(ema_strength, 3), 
                  " 权重=", DoubleToString(ema_weight, 3),
                  " 买入贡献=", DoubleToString(ema_buy_contribution, 4));
        } else {
            // 快线下穿慢线：卖出信号
            double ema_sell_contribution = ema_strength * ema_weight;
            total_sell_strength += ema_sell_contribution;
            Print("📊 EMA信号: 卖出强度=", DoubleToString(ema_strength, 3), 
                  " 权重=", DoubleToString(ema_weight, 3),
                  " 卖出贡献=", DoubleToString(ema_sell_contribution, 4));
        }
        
        total_weight += ema_weight;
        active_technical_weight += ema_weight;
    }
    
    // 2. RSI超买超卖信号 (技术权重20%) - 买入vs卖出强度分配
    double rsi_weight = base_technical_weight * 0.20;
    bool rsi_participated = false;
    
    if(rsi < RSIOversold) {
        // 强超卖：完全买入信号
        double rsi_buy_contribution = 1.0 * rsi_weight;
        total_buy_strength += rsi_buy_contribution;
        rsi_participated = true;
        Print("📊 RSI信号: 强超卖=", DoubleToString(rsi, 1), 
              " 买入贡献=", DoubleToString(rsi_buy_contribution, 4));
              
    } else if(rsi < RSIOversold + 15) {
        // 偏超卖：渐进买入信号
        double rsi_strength = (RSIOversold + 15 - rsi) / 15.0;
        double rsi_buy_contribution = rsi_strength * rsi_weight;
        total_buy_strength += rsi_buy_contribution;
        rsi_participated = true;
        Print("📊 RSI信号: 偏超卖=", DoubleToString(rsi, 1), 
              " 强度=", DoubleToString(rsi_strength, 3),
              " 买入贡献=", DoubleToString(rsi_buy_contribution, 4));
              
    } else if(rsi > RSIOverbought) {
        // 强超买：完全卖出信号
        double rsi_sell_contribution = 1.0 * rsi_weight;
        total_sell_strength += rsi_sell_contribution;
        rsi_participated = true;
        Print("📊 RSI信号: 强超买=", DoubleToString(rsi, 1), 
              " 卖出贡献=", DoubleToString(rsi_sell_contribution, 4));
              
    } else if(rsi > RSIOverbought - 15) {
        // 偏超买：渐进卖出信号
        double rsi_strength = (rsi - (RSIOverbought - 15)) / 15.0;
        double rsi_sell_contribution = rsi_strength * rsi_weight;
        total_sell_strength += rsi_sell_contribution;
        rsi_participated = true;
        Print("📊 RSI信号: 偏超买=", DoubleToString(rsi, 1), 
              " 强度=", DoubleToString(rsi_strength, 3),
              " 卖出贡献=", DoubleToString(rsi_sell_contribution, 4));
              
            } else {
            // 中性区域：增强倾向信号强度（避免权重丢失）
            if(rsi < 45) {
                double rsi_strength = (45 - rsi) / 20.0 * 0.6; // 提高到60%最大强度
                double rsi_buy_contribution = rsi_strength * rsi_weight;
                total_buy_strength += rsi_buy_contribution;
                rsi_participated = true;
                Print("📊 RSI信号: 轻微偏超卖=", DoubleToString(rsi, 1), 
                      " 强度=", DoubleToString(rsi_strength, 3),
                      " 买入贡献=", DoubleToString(rsi_buy_contribution, 4));
                      
            } else if(rsi > 55) {
                double rsi_strength = (rsi - 55) / 20.0 * 0.6; // 提高到60%最大强度
                double rsi_sell_contribution = rsi_strength * rsi_weight;
                total_sell_strength += rsi_sell_contribution;
                rsi_participated = true;
                Print("📊 RSI信号: 轻微偏超买=", DoubleToString(rsi, 1), 
                      " 强度=", DoubleToString(rsi_strength, 3),
                      " 卖出贡献=", DoubleToString(rsi_sell_contribution, 4));
            } else {
                // 中性区域：给予最小基础信号（防止权重完全丢失）
                double rsi_strength = 0.1; // 10%基础强度
                if(rsi < 50) {
                    double rsi_buy_contribution = rsi_strength * rsi_weight;
                    total_buy_strength += rsi_buy_contribution;
                    rsi_participated = true;
                    Print("📊 RSI信号: 中性偏买入=", DoubleToString(rsi, 1), 
                          " 基础买入贡献=", DoubleToString(rsi_buy_contribution, 4));
                } else {
                    double rsi_sell_contribution = rsi_strength * rsi_weight;
                    total_sell_strength += rsi_sell_contribution;
                    rsi_participated = true;
                    Print("📊 RSI信号: 中性偏卖出=", DoubleToString(rsi, 1), 
                          " 基础卖出贡献=", DoubleToString(rsi_sell_contribution, 4));
                }
            }
    }
    ...
    if(rsi_participated) {
        ...
        active_technical_weight += rsi_weight;
    }
    
    // 3. MACD动量信号 (技术权重25%) - 买入vs卖出强度分配 [优化版]
    if(macd_main != 0 && macd_signal_line != 0) {
        // === MACD算法优化 - 专业多维度强度计算 ===
        
        // 1. 基础强度计算 - 降低激进程度
        double histogram_strength = MathMin(MathAbs(macd_histogram) * 1000, 0.8); // 降低系数，最大0.8
        double main_strength = MathMin(MathAbs(macd_main) * 800, 0.6); // 降低系数，最大0.6
        ...
        // 零轴位置质量调整
        if(macd_main > 0) {
            quality_factor *= 1.15; // 零轴上方信号更可靠，+15%
        } else {
            quality_factor *= 0.90; // 零轴下方信号可靠性-10%
        }
        
        // 信号线距离质量调整
        double line_distance = MathAbs(macd_main - macd_signal_line);
        double distance_factor = MathMin(line_distance * 10000, 1.2); // 距离越大，质量越高
        quality_factor *= (0.8 + distance_factor * 0.2); // 基础80% + 距离调整20%
        
        // 柱状图变化率质量调整
        double histogram_magnitude = MathAbs(macd_histogram);
        if(histogram_magnitude > 0.0001) {
            quality_factor *= 1.1; // 柱状图有一定幅度时+10%
        }
        
        // 3. 综合强度计算 - 多因素加权
        double base_strength = (histogram_strength * 0.6 + main_strength * 0.4); // 柱状图60%，主线40%
        double final_strength = MathMin(base_strength * quality_factor, 0.9); // 最终强度上限0.9
        
        double macd_weight = base_technical_weight * 0.25;
        
        // 4. 信号方向判断 - 增加强度分级
        if(macd_main > macd_signal_line && macd_histogram > 0) {
            // 明确买入信号：主线上穿且柱状图为正
            double signal_confidence = 1.0; // 满信心
            double macd_buy_contribution = final_strength * signal_confidence * macd_weight;
            
            // 权重上限控制：单个指标不超过总强度的40%
            double current_total = total_buy_strength + total_sell_strength;
            double max_contribution = (current_total + macd_buy_contribution) * 0.4;
            if(macd_buy_contribution > max_contribution) {
                double original_contribution = macd_buy_contribution;
                macd_buy_contribution = max_contribution;
                Print("📊 MACD权重限制: 买入贡献从", DoubleToString(original_contribution, 4), "限制到", DoubleToString(max_contribution, 4));
            }
            
            total_buy_strength += macd_buy_contribution;
            Print("📊 MACD信号: 明确买入 主线=", DoubleToString(macd_main, 5),
                  " 柱状图=", DoubleToString(macd_histogram, 5),
                  " 基础强度=", DoubleToString(base_strength, 3),
                  " 质量因子=", DoubleToString(quality_factor, 3),
                  " 最终强度=", DoubleToString(final_strength, 3),
                  " 买入贡献=", DoubleToString(macd_buy_contribution, 4));
                  
        } else if(macd_main < macd_signal_line && macd_histogram < 0) {
            // 明确卖出信号：主线下穿且柱状图为负
            double signal_confidence = 1.0; // 满信心
            double macd_sell_contribution = final_strength * signal_confidence * macd_weight;
            
            // 权重上限控制
            double current_total = total_buy_strength + total_sell_strength;
            double max_contribution = (current_total + macd_sell_contribution) * 0.4;
            if(macd_sell_contribution > max_contribution) {
                double original_contribution = macd_sell_contribution;
                macd_sell_contribution = max_contribution;
                Print("📊 MACD权重限制: 卖出贡献从", DoubleToString(original_contribution, 4), "限制到", DoubleToString(max_contribution, 4));
            }
            
            total_sell_strength += macd_sell_contribution;
            Print("📊 MACD信号: 明确卖出 主线=", DoubleToString(macd_main, 5),
                  " 柱状图=", DoubleToString(macd_histogram, 5),
                  " 基础强度=", DoubleToString(base_strength, 3),
                  " 质量因子=", DoubleToString(quality_factor, 3),
                  " 最终强度=", DoubleToString(final_strength, 3),
                  " 卖出贡献=", DoubleToString(macd_sell_contribution, 4));
                  
        } else if(macd_histogram > 0 && histogram_magnitude > 0.00005) {
            // 轻微买入倾向：仅柱状图为正且有一定幅度
            double signal_confidence = 0.4; // 降低信心到40%
            double macd_buy_contribution = final_strength * signal_confidence * macd_weight;
            
            total_buy_strength += macd_buy_contribution;
            Print("📊 MACD信号: 轻微买入 柱状图=", DoubleToString(macd_histogram, 5),
                  " 最终强度=", DoubleToString(final_strength, 3),
                  " 信心度=", DoubleToString(signal_confidence, 2),
                  " 买入贡献=", DoubleToString(macd_buy_contribution, 4));
            ...
            total_sell_strength += macd_sell_contribution;
            Print("📊 MACD信号: 轻微卖出 柱状图=", DoubleToString(macd_histogram, 5),
                  " 最终强度=", DoubleToString(final_strength, 3),
                  " 信心度=", DoubleToString(signal_confidence, 2),
                  " 卖出贡献=", DoubleToString(macd_sell_contribution, 4));
        } else {
            Print("📊 MACD信号: 中性/弱信号 主线=", DoubleToString(macd_main, 5), 
                  " 柱状图=", DoubleToString(macd_histogram, 5),
                  " 柱状图幅度=", DoubleToString(histogram_magnitude, 6));
        }
        
        total_weight += macd_weight;
        active_technical_weight += macd_weight;
    }
    
    // 4. 布林带挤压和突破信号 (技术权重5%) - 买入vs卖出强度分配
    if(bb_upper != 0 && bb_lower != 0 && bb_middle != 0) {
        double current_price = Close[0];
        double bb_width = bb_upper - bb_lower;
        double bb_weight = base_technical_weight * 0.05;
        bool bb_participated = false;
        
        if(bb_width > 0) {
            double bb_position = (current_price - bb_lower) / bb_width; // 0-1位置
            
            // 布林带买入/卖出信号分配
            if(current_price <= bb_lower) {
                // 触及下轨：强买入信号
                double bb_buy_contribution = 1.0 * bb_weight;
                total_buy_strength += bb_buy_contribution;
                bb_participated = true;
                Print("📊 布林带信号: 触及下轨 位置=", DoubleToString(bb_position, 3),
                      " 买入贡献=", DoubleToString(bb_buy_contribution, 4));
                      
            } else if(current_price >= bb_upper) {
                // 触及上轨：强卖出信号
                double bb_sell_contribution = 1.0 * bb_weight;
                total_sell_strength += bb_sell_contribution;
                bb_participated = true;
                Print("📊 布林带信号: 触及上轨 位置=", DoubleToString(bb_position, 3),
                      " 卖出贡献=", DoubleToString(bb_sell_contribution, 4));
                      
            } else if(bb_position < 0.2) {
                // 接近下轨：买入倾向
                double bb_strength = (0.2 - bb_position) / 0.2;
                double bb_buy_contribution = bb_strength * bb_weight;
                total_buy_strength += bb_buy_contribution;
                bb_participated = true;
                Print("📊 布林带信号: 接近下轨 位置=", DoubleToString(bb_position, 3),
                      " 强度=", DoubleToString(bb_strength, 3),
                      " 买入贡献=", DoubleToString(bb_buy_contribution, 4));
                      
            } else if(bb_position > 0.8) {
                // 接近上轨：卖出倾向
                double bb_strength = (bb_position - 0.8) / 0.2;
                double bb_sell_contribution = bb_strength * bb_weight;
                total_sell_strength += bb_sell_contribution;
                bb_participated = true;
                Print("📊 布林带信号: 接近上轨 位置=", DoubleToString(bb_position, 3),
                      " 强度=", DoubleToString(bb_strength, 3),
                      " 卖出贡献=", DoubleToString(bb_sell_contribution, 4));
            } else {
                // 中性区域：根据距离中心的偏离给予轻微信号
                double distance_from_center = MathAbs(bb_position - 0.5); // 距离中心的偏移
                if(distance_from_center > 0.05) { // 降低阈值从0.1到0.05
                    double bb_strength = distance_from_center * 0.4; // 40%最大强度
                    if(bb_position > 0.5) {
                        // 偏向上轨：轻微卖出倾向
                        double bb_sell_contribution = bb_strength * bb_weight;
                        total_sell_strength += bb_sell_contribution;
                        bb_participated = true;
                        Print("📊 布林带信号: 中性偏上 位置=", DoubleToString(bb_position, 3),
                              " 强度=", DoubleToString(bb_strength, 3),
                              " 卖出贡献=", DoubleToString(bb_sell_contribution, 4));
                    } else {
                        // 偏向下轨：轻微买入倾向
                        ...
                        total_buy_strength += bb_buy_contribution;
                        bb_participated = true;
                        Print("📊 布林带信号: 中性偏下 位置=", DoubleToString(bb_position, 3),
                              " 强度=", DoubleToString(bb_strength, 3),
                              " 买入贡献=", DoubleToString(bb_buy_contribution, 4));
                    }
                } else {
                    Print("📊 布林带信号: 完全中性区域 位置=", DoubleToString(bb_position, 3));
                }
            }
        }
        ...
        if(bb_participated) {
            ...
            active_technical_weight += bb_weight;
        }
    }
    
    // 5. ATR波动率信号 (技术权重20%) - 买入vs卖出强度分配
    if(atr > 0) {
        double avg_atr = GetAverageATR(14);
        if(avg_atr > 0) {
            double atr_ratio = atr / avg_atr;
            double atr_weight = base_technical_weight * 0.20;
            bool atr_participated = false;
            
            // ATR信号：低波动偏向买入，高波动偏向风险警告
            if(atr_ratio < 0.8) {
                // 低波动：买入机会
                double atr_strength = (0.8 - atr_ratio) / 0.3; // 0.5为满强度
                atr_strength = MathMin(atr_strength, 1.0);
                double atr_buy_contribution = atr_strength * atr_weight;
                total_buy_strength += atr_buy_contribution;
                atr_participated = true;
                Print("📊 ATR信号: 低波动机会 比率=", DoubleToString(atr_ratio, 3),
                      " 强度=", DoubleToString(atr_strength, 3),
                      " 买入贡献=", DoubleToString(atr_buy_contribution, 4));
                      
            } else if(atr_ratio > 1.5) {
                // 高波动：风险警告，偏向卖出
                double atr_strength = MathMin((atr_ratio - 1.5) / 1.0, 1.0); // 2.5为满强度
                double atr_sell_contribution = atr_strength * atr_weight;
                total_sell_strength += atr_sell_contribution;
                atr_participated = true;
                Print("📊 ATR信号: 高波动警告 比率=", DoubleToString(atr_ratio, 3),
                      " 强度=", DoubleToString(atr_strength, 3),
                      " 卖出贡献=", DoubleToString(atr_sell_contribution, 4));
                      
            } else {
                // 正常波动：轻微倾向低波动一侧
                if(atr_ratio < 1.0) {
                    double atr_strength = (1.0 - atr_ratio) / 0.2 * 0.3; // 最大30%强度
                    double atr_buy_contribution = atr_strength * atr_weight;
                    total_buy_strength += atr_buy_contribution;
                    atr_participated = true;
                    Print("📊 ATR信号: 轻微低波动 比率=", DoubleToString(atr_ratio, 3),
                          " 强度=", DoubleToString(atr_strength, 3),
                          " 买入贡献=", DoubleToString(atr_buy_contribution, 4));
                } else {
                    double atr_strength = (atr_ratio - 1.0) / 0.5 * 0.3; // 最大30%强度
                    double atr_sell_contribution = atr_strength * atr_weight;
                    total_sell_strength += atr_sell_contribution;
                    atr_participated = true;
                    Print("📊 ATR信号: 轻微高波动 比率=", DoubleToString(atr_ratio, 3),
                          " 强度=", DoubleToString(atr_strength, 3),
                          " 卖出贡献=", DoubleToString(atr_sell_contribution, 4));
                }
            }
            ...
            if(atr_participated) {
                total_weight += atr_weight;
                active_technical_weight += atr_weight;
            }
        }
    }
    
    // === 动态权重重新分配机制 ===
    double unused_technical_weight = base_technical_weight - active_technical_weight;
    if(unused_technical_weight > 0.001) {
        Print("📊 权重重新分配: 未使用技术权重=", DoubleToString(unused_technical_weight, 3));
        // 将未使用的技术权重转移给市场权重
        base_market_weight += unused_technical_weight;
        Print("📊 调整后市场权重: ", DoubleToString(base_market_weight, 3));
    }
    
    // === 市场状况买入/卖出强度分配 ===
    if(market_score > 50) {
        // 看涨市场：支持买入
        double market_strength = (market_score - 50) / 50.0; // 0-1强度
        double market_buy_contribution = market_strength * base_market_weight;
        total_buy_strength += market_buy_contribution;
        
        Print("📊 市场信号: 看涨市场 评分=", DoubleToString(market_score, 1),
              " 强度=", DoubleToString(market_strength, 3),
              " 买入贡献=", DoubleToString(market_buy_contribution, 4));
              
    } else if(market_score < 50) {
        // 看跌市场：支持卖出
        double market_strength = (50 - market_score) / 50.0; // 0-1强度
        double market_sell_contribution = market_strength * base_market_weight;
        total_sell_strength += market_sell_contribution;
        
        Print("📊 市场信号: 看跌市场 评分=", DoubleToString(market_score, 1),
              " 强度=", DoubleToString(market_strength, 3),
              " 卖出贡献=", DoubleToString(market_sell_contribution, 4));
    } else {
        Print("📊 市场信号: 完全中性 评分=", DoubleToString(market_score, 1));
    }
    
    total_weight += base_market_weight;
    
    // === 价格位置买入/卖出强度分配（5%额外权重）===
    double position_weight = 0.05; // 固定5%权重
    bool position_participated = false;
    
    if(pos.position_50 < 30.0) {
        // 低位：强买入信号
        double position_strength = (30.0 - pos.position_50) / 30.0;
        double position_buy_contribution = position_strength * position_weight;
        total_buy_strength += position_buy_contribution;
        position_participated = true;
        
        Print("📊 位置信号: 低位买入 位置=", DoubleToString(pos.position_50, 1), "%",
              " 强度=", DoubleToString(position_strength, 3),
              " 买入贡献=", DoubleToString(position_buy_contribution, 4));
              
    } else if(pos.position_50 > 70.0) {
        // 高位：强卖出信号
        double position_strength = (pos.position_50 - 70.0) / 30.0;
        double position_sell_contribution = position_strength * position_weight;
        total_sell_strength += position_sell_contribution;
        position_participated = true;
        
        Print("📊 位置信号: 高位卖出 位置=", DoubleToString(pos.position_50, 1), "%",
              " 强度=", DoubleToString(position_strength, 3),
              " 卖出贡献=", DoubleToString(position_sell_contribution, 4));
              
    } else {
        // 中性位置：轻微倾向
        if(pos.position_50 < 50.0) {
            double position_strength = (50.0 - pos.position_50) / 20.0 * 0.3;
            double position_buy_contribution = position_strength * position_weight;
            total_buy_strength += position_buy_contribution;
            position_participated = true;
            
            Print("📊 位置信号: 轻微低位买入 位置=", DoubleToString(pos.position_50, 1), "%",
                  " 强度=", DoubleToString(position_strength, 3),
                  " 买入贡献=", DoubleToString(position_buy_contribution, 4));
                  
        } else if(pos.position_50 > 50.0) {
            double position_strength = (pos.position_50 - 50.0) / 20.0 * 0.3;
            double position_sell_contribution = position_strength * position_weight;
            total_sell_strength += position_sell_contribution;
            position_participated = true;
            
            Print("📊 位置信号: 轻微高位卖出 位置=", DoubleToString(pos.position_50, 1), "%",
                  " 强度=", DoubleToString(position_strength, 3),
                  " 卖出贡献=", DoubleToString(position_sell_contribution, 4));
        } else {
            Print("📊 位置信号: 完全中性 位置=", DoubleToString(pos.position_50, 1), "%");
        }
    }
    
    if(position_participated) {
        total_weight += position_weight;
    }
    
    // === 最终买入vs卖出概率分配计算 ===
    Print("📊 强度汇总:");
    Print("   买入总强度: ", DoubleToString(total_buy_strength, 4));
    Print("   卖出总强度: ", DoubleToString(total_sell_strength, 4));
    Print("   总权重: ", DoubleToString(total_weight, 4));
    
    // 基于买入vs卖出强度竞争的概率分配
    double total_strength = total_buy_strength + total_sell_strength;
    
    if(total_strength > 0) {
        // 标准化为概率分配：买入强度/(买入强度+卖出强度)
        buy_score = total_buy_strength / total_strength;
        sell_score = total_sell_strength / total_strength;
        
        Print("📊 竞争式概率分配:");
        Print("   买入强度占比: ", DoubleToString(buy_score, 4));
        Print("   卖出强度占比: ", DoubleToString(sell_score, 4));
        ...
        Print("📊 无信号状态: 默认中性概率分配");
    }
    
    // 验证概率和为1.0（数学上自动满足）
    double total_probability = buy_score + sell_score;
    if(MathAbs(total_probability - 1.0) > 0.001) {
        Print("⚠️ 概率和校正: ", DoubleToString(total_probability, 6), " → 1.0");
        buy_score = buy_score / total_probability;
        sell_score = sell_score / total_probability;
    }
    
    Print("📊 最终买入vs卖出概率:");
    Print("   买入概率: ", DoubleToString(buy_score, 4), " (", DoubleToString(buy_score * 100, 1), "%)");
    Print("   卖出概率: ", DoubleToString(sell_score, 4), " (", DoubleToString(sell_score * 100, 1), "%)");
    Print("   概率验证: ", DoubleToString(total_probability, 6), " (=1.0)");
    Print("   权重分布 - 技术:", DoubleToString(active_technical_weight, 3), 
          " 市场:", DoubleToString(base_market_weight, 3), 
          " 位置:", DoubleToString(position_weight, 3));
    Print("   优势方向: ", (buy_score > sell_score ? "买入优势" : "卖出优势"), 
          " 优势幅度:", DoubleToString(MathAbs(buy_score - sell_score), 4));
    
    // === 概率分配计算完成 ===
    // 移除旧的累加制代码，避免双重计算
    /* 
    // 以下所有代码已被新的概率分配系统替代，注释保留备用
    // 暂时注释掉原来的高级技术指标代码
    if(EnableAdvancedIndicators) {
        double bb_position = GetBollingerPosition();
        if(bb_position > BollingerOverbought) {
            technical_sell_score += final_technical_weight * 5.0; // 5分满分
            Print("📊 布林带信号: 超买 ", DoubleToString(bb_position, 1), "% → 卖出评分 +", DoubleToString(final_technical_weight * 5.0, 1));
        } else if(bb_position < BollingerOversold) {
            technical_buy_score += final_technical_weight * 5.0; // 5分满分
            Print("📊 布林带信号: 超卖 ", DoubleToString(bb_position, 1), "% → 买入评分 +", DoubleToString(final_technical_weight * 5.0, 1));
        } else {
        ...
        if(macd_signal == 1) {
            technical_buy_score += final_technical_weight * 25.0 * macd_score_factor; // 25分满分
            Print("📊 MACD信号: 看涨强度", DoubleToString(macd_score_factor, 3), " → 买入评分 +", DoubleToString(final_technical_weight * 25.0 * macd_score_factor, 1), " (主=", DoubleToString(macd_main, 5), " 柱=", DoubleToString(macd_histogram, 5), ")");
        } else if(macd_signal == -1) {
            technical_sell_score += final_technical_weight * 25.0 * macd_score_factor; // 25分满分
            Print("📊 MACD信号: 看跌强度", DoubleToString(macd_score_factor, 3), " → 卖出评分 +", DoubleToString(final_technical_weight * 25.0 * macd_score_factor, 1), " (主=", DoubleToString(macd_main, 5), " 柱=", DoubleToString(macd_histogram, 5), ")");
        } else if(macd_histogram > 0) {
            // 即使没有明确信号，柱状图为正也给予轻微买入倾向
            technical_buy_score += final_technical_weight * 25.0 * macd_score_factor * 0.3; // 7.5分满分
            Print("📊 MACD信号: 微弱看涨 → 买入评分 +", DoubleToString(final_technical_weight * 25.0 * macd_score_factor * 0.3, 1), " (柱=", DoubleToString(macd_histogram, 5), ")");
        } else if(macd_histogram < 0) {
            // 即使没有明确信号，柱状图为负也给予轻微卖出倾向
            technical_sell_score += final_technical_weight * 25.0 * macd_score_factor * 0.3; // 7.5分满分
            Print("📊 MACD信号: 微弱看跌 → 卖出评分 +", DoubleToString(final_technical_weight * 25.0 * macd_score_factor * 0.3, 1), " (柱=", DoubleToString(macd_histogram, 5), ")");
        } else {
        ...
        if(atr_value > atr_avg * 1.2) {
            technical_buy_score += final_technical_weight * 20.0; // 20分满分
            technical_sell_score += final_technical_weight * 20.0; // 20分满分
            Print("📊 ATR信号: 高波动期 ", DoubleToString(atr_value, 2), " > ", DoubleToString(atr_avg * 1.2, 2), " → 双向评分 +", DoubleToString(final_technical_weight * 20.0, 1));
    ...
    // 技术评分标准化到100分制 - 高频交易优化
    double max_technical_score = final_technical_weight * 100.0; // 技术部分最大100分
    ...
    if(market_suitable)
    {
        ...
        // 市场评分标准化到100分制 - 高频交易优化
        double max_market_score = final_market_weight * 100.0; // 市场部分最大100分
    ...
    */
    Print("🔍 概率分配决策评分计算完成 - 时间: ", TimeToString(TimeCurrent()));
}
