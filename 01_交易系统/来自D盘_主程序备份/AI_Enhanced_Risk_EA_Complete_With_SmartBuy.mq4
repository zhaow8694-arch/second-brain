...
// 输入参数
input int MaxOpenOrders = 10;             // 最大同时开仓数 (调整为10，支持四种仓位类型)
...
input int MaxSameDirectionOrders = 8;      // 同方向最大持仓数 (普通仓位4个 + 应急仓位2个 + 智能加仓2个)
...
input int MaxNormalOrders = 4;             // 普通仓最多4个持仓
...
input double SmartBuyAIConfidenceThreshold = 0.3;  // AI置信度阈值
input int MaxSmartBuyOrders = 2;              // 智能加仓最大订单数（不影响4个正常订单）
input double MaxSmartBuyTotalLots = 0.03;     // 智能加仓最大总手数（0.03手）
input int SmartBuyMinInterval = 300;          // 智能加仓最小间隔（5分钟）
input double SmartBuyLossThreshold = 50.0;   // 智能加仓触发亏损阈值（点数）
...
public:
    ...
    void UpdateIndicators()
    {
        ...
        static int indicator_tick_count = 0;
        indicator_tick_count++;
        
        if(current_time != last_update_time || indicator_tick_count >= 10)
        {
            // 批量计算所有指标，减少函数调用开销 - 优化MA类型选择
                    cached_ma_fast = iMA(Symbol(), Period(), 15, 0, MODE_EMA, PRICE_CLOSE, 0);  // 快线使用EMA更敏感
        cached_ma_slow = iMA(Symbol(), Period(), 30, 0, MODE_SMA, PRICE_CLOSE, 0);  // 慢线使用SMA更稳定
        ...
        cached_ma_200 = iMA(Symbol(), Period(), 200, 0, MODE_SMA, PRICE_CLOSE, 0);  // 200周期MA
            ...
            indicator_tick_count = 0; // 重置tick计数
    ...
    {
        UpdateIndicators();
        if(period == 15) return cached_ma_fast;  // 15周期EMA
        else if(period == 30) return cached_ma_slow;  // 30周期SMA
...
public:
    ...
    // 新增：获取所有EA订单的ticket数组
    void GetAllOrderTickets(int &tickets[])
    {
...
public:
    ...
    static bool CheckTrailingStop(int ticket)
    {
        ...
        double profit_pips = current_profit / (Point * 10); // 修复：正确计算点位
...
public:
    ...
    void ManageLockedPositions()
    {
        ...
        double total_pips = total_profit / Point;
        ...
        for(int i = 0; i < OrdersTotal(); i++)
        {
            if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
            {
                if(OrderSymbol() == Symbol() && OrderMagicNumber() == 12345)
                {
                    ...
                    if(current_profit < 0)
                    {
                        double loss_pips = MathAbs(current_profit) / Point;
                        int ticket = OrderTicket();
    ...
    void ExecuteLockOrder(int order_type, string comment)
    {
        ...
        int ticket = OrderSend(Symbol(), order_type, lot_size, price, (int)MaxSlippage, 0, 0, comment, 12345, 0, clrNONE);
        ...
        for(int i = 0; i < OrdersTotal(); i++)
        {
            if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
            {
                if(OrderSymbol() == Symbol() && OrderMagicNumber() == 12345)
                {
                    ...
                    {
                        double profit = ::GetCurrentOrderTotalProfit();
                        double profit_pips = profit / (Point * 10); // 修复：正确计算点位
                        int ticket = OrderTicket();
...
private:
    int MakeFinalDecision(int ai_prediction, double ai_confidence, bool has_ai_signal, bool market_suitable, double buy_score, double sell_score)
    {
        ...
        // 最终决策 - 提高阈值以确保高质量信号
        double decision_threshold = 0.6; // 调整为0.6，确保高质量建仓
    ...
    void ExecuteOrder(int order_type)
    {
        ...
        if(order_type == OP_BUY)
        {
            ...
            int ticket = OrderSend(Symbol(), OP_BUY, lot_size, Ask, (int)MaxSlippage, 0.0, 0.0, 
        ...
        else if(order_type == OP_SELL)
        {
            ...
            int sell_ticket = OrderSend(Symbol(), OP_SELL, lot_size, Bid, (int)MaxSlippage, 0.0, 0.0, 
            ...
            else
            {
                ...
                Print("卖出订单执行失败，错误代码: ", sell_error);
        ...
        for(int idx2 = OrdersTotal() - 1; idx2 >= 0; idx2--)
        {
            if(OrderSelect(idx2, SELECT_BY_POS, MODE_TRADES))
            {
                if(IsOurOrder())
                {
                    ...
                    {
                        Print("🔒 智能加仓订单 ", OrderTicket(), " 跳过其他平仓逻辑，只使用智能加仓专用平仓");
                        continue;
                    }
...
private:
    ...
    // 决策评分反转平仓检查 - 完全禁用反转信号平仓
    bool CheckDecisionReversalClose(int ticket)
    {
        // 🚫 修改：锁仓单不给予反转信号平仓
        // 所有仓位（包括锁仓单）都不执行反转信号平仓，完全扛单模式
    ...
    bool CheckTechnicalCloseCondition(int ticket)
    {
        ...
        // 使用缓存的技术指标 - 性能优化
        double ma_fast = indicator_cache.GetMA(15);  // 更新为15周期
        double ma_slow = indicator_cache.GetMA(30);  // 更新为30周期
        ...
        if(OrderType() == OP_BUY && rsi > RSIOverbought) {
            if(profit_pips > 0 && rsi < 98.0) return false;
            // 扛单策略：亏损订单不因RSI超买平仓
            if(EnableHoldStrategy && profit_pips < 0) return false;
            if(ai_ok && ai_confidence > 0.8 && ai_prediction == 2) return false;
            ...
            if(profit_pips > 0 && rsi > 2.0) return false;
            // 扛单策略：亏损订单不因RSI超卖平仓
            if(EnableHoldStrategy && profit_pips < 0) return false;
            if(ai_ok && ai_confidence > 0.8 && ai_prediction == 0) return false;
    ...
    bool CheckLossManagement(int ticket)
    {
        ...
        double loss_pips = MathAbs(current_profit) / (Point * 10); // 修复：正确计算点位
    ...
    bool CheckKeyLevelSupport(int ticket, double loss_pips)
    {
        ...
        {
            // 买入订单：接近支撑位
            double distance_to_support = (current_price - support_level) / Point;
        ...
        {
            // 卖出订单：接近阻力位
            double distance_to_resistance = (resistance_level - current_price) / Point;
...
{
    Print("=== AI增强风险管理EA启动 ===");
    Print("版本: 2.8.1 - 锁仓系统版 🔒");
    Print("🔧 关键升级:");
            Print("  🔒 锁仓: 3000点统一触发");
            Print("  📊 锁仓手数: 1.0倍基础手数");
            Print("  💰 锁仓解锁: 300点盈利解锁");
    Print("  📈 持仓上限: 4单普通 + 2单锁仓");
    Print("  ⏰ 建仓频率: 15分钟间隔控制");
    Print("🔧 核心功能:");
    Print("  ✅ 扛单策略: 10000点极限，无时间限制");
            Print("  ✅ 锁仓: 3000点触发，300点解锁");
    Print("  ✅ 盈利目标: 400点获利，移动止损1.0倍ATR保护");
    Print("  ✅ 账户保护: 30%强制平仓保护");
    Print("  ✅ AI决策权重: 30%AI + 40%技术 + 40%市场");
    ...
    Print("同方向最大持仓: ", MaxSameDirectionOrders, " (普通仓位4个 + 应急仓位2个 + 智能加仓2个)");
    Print("=== 手数设置 ===");
    ...
    Print("AI权重降低因子: ", AIWeightReductionFactor);
...
void CalculateDecisionScores(double &buy_score, double &sell_score)
{
    ...
    // 基础技术分析评分 - 使用统一的RSI阈值
    if(ma_fast > ma_slow) {
        technical_buy_score += TechnicalWeight;
        Print("📊 MA信号: 快线(", DoubleToString(ma_fast, 5), ") > 慢线(", DoubleToString(ma_slow, 5), ") → 买入评分 +", DoubleToString(TechnicalWeight, 2));
    } else {
        technical_sell_score += TechnicalWeight;
        Print("📊 MA信号: 快线(", DoubleToString(ma_fast, 5), ") < 慢线(", DoubleToString(ma_slow, 5), ") → 卖出评分 +", DoubleToString(TechnicalWeight, 2));
    }
    
    if(rsi < RSIOversold) {
        technical_buy_score += TechnicalWeight * 0.5;
        Print("📊 RSI信号: 超卖 ", DoubleToString(rsi, 1), " < ", DoubleToString(RSIOversold, 1), " → 买入评分 +", DoubleToString(TechnicalWeight * 0.5, 2));
    } else if(rsi > RSIOverbought) {
        technical_sell_score += TechnicalWeight * 0.5;
        Print("📊 RSI信号: 超买 ", DoubleToString(rsi, 1), " > ", DoubleToString(RSIOverbought, 1), " → 卖出评分 +", DoubleToString(TechnicalWeight * 0.5, 2));
    } else {
        Print("📊 RSI信号: 中性 ", DoubleToString(rsi, 1));
    }
    
            // 高级技术指标评分 - 新增集成（增强日志输出）
        if(EnableAdvancedIndicators)
        {
            ...
            {
                technical_sell_score += TechnicalWeight * 0.3;
                Print("📊 布林带信号: 超买 ", DoubleToString(bb_position, 1), "% → 卖出评分 +", DoubleToString(TechnicalWeight * 0.3, 2));
            }
            else if(bb_position < BollingerOversold) 
            {
                technical_buy_score += TechnicalWeight * 0.3;
                Print("📊 布林带信号: 超卖 ", DoubleToString(bb_position, 1), "% → 买入评分 +", DoubleToString(TechnicalWeight * 0.3, 2));
            }
            else
            {
            ...
            if(macd_signal == 1) 
            {
                technical_buy_score += TechnicalWeight * 1.0; // 提高MACD权重到100%
                Print("📊 MACD信号: 看涨 → 买入评分 +", DoubleToString(TechnicalWeight * 1.0, 2), " (主=", DoubleToString(macd_main, 5), " 信号=", DoubleToString(macd_signal_line, 5), " 柱=", DoubleToString(macd_histogram, 5), ")");
            }
            else if(macd_signal == -1) 
            {
                technical_sell_score += TechnicalWeight * 1.0; // 提高MACD权重到100%
                Print("📊 MACD信号: 看跌 → 卖出评分 +", DoubleToString(TechnicalWeight * 1.0, 2), " (主=", DoubleToString(macd_main, 5), " 信号=", DoubleToString(macd_signal_line, 5), " 柱=", DoubleToString(macd_histogram, 5), ")");
            }
            else
            {
            ...
            double kdj_k = advanced_indicators.GetKValue();
            double kdj_d = advanced_indicators.GetDValue();
            double kdj_j = advanced_indicators.GetJValue();
            
            if(kdj_signal == 1) 
            {
                technical_buy_score += TechnicalWeight * 0.3;
                Print("📊 KDJ信号: 超卖 → 买入评分 +", DoubleToString(TechnicalWeight * 0.3, 2), " (K=", DoubleToString(kdj_k, 1), " D=", DoubleToString(kdj_d, 1), " J=", DoubleToString(kdj_j, 1), ")");
            }
            else if(kdj_signal == -1) 
            {
                technical_sell_score += TechnicalWeight * 0.3;
                Print("📊 KDJ信号: 超买 → 卖出评分 +", DoubleToString(TechnicalWeight * 0.3, 2), " (K=", DoubleToString(kdj_k, 1), " D=", DoubleToString(kdj_d, 1), " J=", DoubleToString(kdj_j, 1), ")");
            }
            else
            {
                Print("📊 KDJ信号: 中性 (K=", DoubleToString(kdj_k, 1), " D=", DoubleToString(kdj_d, 1), " J=", DoubleToString(kdj_j, 1), ")");
            }
        }
    
    // 限制技术评分最大值 - 修复权重累加问题
    double max_technical_score = TechnicalWeight * 2.0; // 最大技术评分为权重的2.0倍，适应多个指标叠加
    ...
    if(market_suitable)
    {
        ...
        // 基础市场评分 - 修复：根据市场评分分配权重，确保平衡
        if(market_score > 50) {
            // 市场评分高于50，偏向买入
            market_buy_score += MarketWeight * ((market_score - 50) / 50.0);
            Print("📈 市场评分: 看涨 ", DoubleToString(market_score, 1), " → 买入评分 +", DoubleToString(MarketWeight * ((market_score - 50) / 50.0), 2));
        } else {
            // 市场评分低于50，偏向卖出
            market_sell_score += MarketWeight * ((50 - market_score) / 50.0);
            Print("📉 市场评分: 看跌 ", DoubleToString(market_score, 1), " → 卖出评分 +", DoubleToString(MarketWeight * ((50 - market_score) / 50.0), 2));
        }
        ...
        if(EnableAdvancedMarketAnalysis)
        {
            ...
            // 根据市场状态和高级评分决定方向 - 确保平衡
            if(market_state == 1) // 强势市场
            {
                // 强势市场偏向买入
                double bonus = MarketWeight * (advanced_market_score / 100.0) * 1.1;
            ...
            else // 震荡市场
            {
                ...
                if(advanced_market_score > 50) {
                    double bonus = MarketWeight * (advanced_market_score / 100.0);
                    market_buy_score += bonus;
                    Print("📊 高级市场分析: 震荡市场看涨 → 买入评分 +", DoubleToString(bonus, 2), " (评分=", DoubleToString(advanced_market_score, 1), ")");
                } else {
                    double bonus = MarketWeight * ((100 - advanced_market_score) / 100.0);
                    market_sell_score += bonus;
        ...
        // 限制市场评分最大值 - 避免重复计算
        double max_market_score = MarketWeight * 1.2; // 最大市场评分为权重的1.2倍
    ...
    // AI预测评分（如果可用且置信度足够）- 新增位置感知动态权重调整
    int ai_prediction = -1;
    double ai_confidence = 0.0;
    bool has_ai_signal = ai_predictor.GetAIPrediction(ai_prediction, ai_confidence);
    ...
    Print("📊 技术评分 - 买入: ", DoubleToString(technical_buy_score, 2), " 卖出: ", DoubleToString(technical_sell_score, 2));
    if(market_suitable) {
        Print("📈 市场评分 - 基础: ", DoubleToString(market_score, 2));
    }
    if(has_ai_signal && ai_confidence >= MinAIConfidence) {
...
void UpdateSmartBuyStats()
{
    ...
    for(int i = 0; i < OrdersTotal(); i++)
    {
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
        {
            ...
            {
                string comment = OrderComment();
                Print("📋 检查订单 ", OrderTicket(), " 注释: '", comment, "'");
                
                if(StringFind(comment, "智能加仓") >= 0)
                {
                    g_smart_buy_orders++;
                    g_smart_buy_total_lots += OrderLots();
                    Print("✅ 识别为智能加仓订单: ", OrderTicket(), " 手数: ", OrderLots());
                }
                else
                {
                    Print("❌ 非智能加仓订单: ", OrderTicket(), " 注释: '", comment, "'");
                }
            }
        }
    }
    
    Print("📊 智能加仓统计完成: ", g_smart_buy_orders, " 订单, ", g_smart_buy_total_lots, " 手数");
...
void CheckAndExecuteSmartBuy(double ai_confidence, double buy_score, double sell_score, int ai_prediction = -1)
{
    ...
    // 使用传入的AI置信度参数，不再重新获取AI预测
    Print("📊 智能加仓检查 - 使用传入的AI置信度: ", ai_confidence);
    ...
    // 根据参数决定是否要求方向一致性
    if(SmartBuyRequireDirectionMatch && !ai_direction_match)
    {
        Print("📊 AI预测方向与主导亏损方向不一致，跳过智能加仓");
        Print("   主导亏损方向: ", (dominant_direction == OP_BUY ? "买入" : "卖出"));
        Print("   AI预测方向: ", (ai_prediction == 2 ? "买入" : (ai_prediction == 0 ? "卖出" : "中性")));
        Print("   ⚙️ 当前设置：要求方向一致性");
        return;
    }
    else if(!ai_direction_match)
    {
        Print("📊 AI预测方向与主导亏损方向不一致，但继续执行智能加仓");
        Print("   主导亏损方向: ", (dominant_direction == OP_BUY ? "买入" : "卖出"));
        Print("   AI预测方向: ", (ai_prediction == 2 ? "买入" : (ai_prediction == 0 ? "卖出" : "中性")));
        Print("   ⚠️ 注意：方向不一致可能增加风险");
        Print("   ⚙️ 当前设置：不要求方向一致性");
    }
    else
    {
        Print("📊 AI预测方向与主导亏损方向一致，风险较低");
    }
