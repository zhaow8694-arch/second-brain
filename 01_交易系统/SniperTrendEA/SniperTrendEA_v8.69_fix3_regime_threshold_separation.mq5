...
input string InpComment = "SniperEA_v8.69_fix3";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.69_fix3_regime_threshold_separation";
...
double GetV869IndicatorValue(int handle, int bufferIndex, int shift)
{
   ...
   if(copied <= 0)
   {
      if(InpRegimeDebugLog)
         Print("[v8.69_fix3 RegimeGate] CopyBuffer failed | handle=", handle,
...
double GetV869RegimeScore()
{
   ...
   if(InpRegimeDebugLog)
      Print("[v8.69_fix3 RegimeGate] score=", DoubleToString(score, 1),
...
double GetV869RegimeRiskMultiplier()
{
   ...
   if(InpRegimeDebugLog)
      Print("[v8.69_fix3 RegimeGate] multiplier=", DoubleToString(multiplier, 2),
...
double AdjustLotSize(double lot)
{
   ...
   {
      if(InpRegimeDebugLog)
         Print("[v8.69_fix3 RegimeGate] new entry blocked by regime multiplier");
      return 0.0;
   }
