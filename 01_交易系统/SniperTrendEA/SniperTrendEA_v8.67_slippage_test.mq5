...
input int    InpSlippagePressurePips = 0; // temp slippage pressure injection (unittest only)
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Temporary slippage simulation helper (test only)                    |
//+------------------------------------------------------------------+
double ApplySlippagePrice(const double price, const ENUM_ORDER_TYPE type, const int slippagePips)
{
   if(slippagePips <= 0) return price;
   double shift = slippagePips * _Point;
   double out = (type == ORDER_TYPE_BUY) ? (price + shift) : (price - shift);
...
bool OpenPosition(ENUM_ORDER_TYPE type, double price, double sl, double lot)
{
   ...
   req.type = type; double intendedPrice = price;
   double filledPrice = ApplySlippagePrice(intendedPrice, type, InpSlippagePressurePips);
   if(InpSlippagePressurePips > 0) Print("[slippage test] inject pips=", InpSlippagePressurePips, " | intend=", DoubleToString(intendedPrice, _Digits), " | fill=", DoubleToString(filledPrice, _Digits));
   req.price = filledPrice; req.sl = sl; req.tp = 0;
