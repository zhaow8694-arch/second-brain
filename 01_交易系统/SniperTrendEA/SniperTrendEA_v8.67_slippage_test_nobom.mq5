...
//--- 杈撳叆鍙傛暟
input group "=== 杩囨护寮哄害棰勮锛坴8.61锛?=="
input ENUM_FILTER_PRESET InpFilterPreset = FILTER_AGGRESSIVE; // 杩囨护寮哄害锛堝榻?grok8.6 閿氱偣榛樿=2锛?
...
input string InpComment        = "SniperEA_v8.67";
...
input group "=== Slippage Harness (Temp only) ==="
input bool   InpEnableSlippageHarness = false;
input int    InpSlippageInjectionPips = 0;
input int    InpSlippageHarnessSeed = 20260620;
input int    InpSlippageRetryLimit = 2;
input bool   InpLogSlippageTrades = true;
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.67_slippage_test";
const string EA_PARAMETER_PROFILE = "robust_main_case0010";
const string EA_RECOMMENDED_SET   = "v8.66_robust_main_case0010.set";
int g_slippageRngState       = 0;
int g_lastSlippageAppliedPts  = 0;
int g_lastSlippageDirection  = 0;
...
{
   g_slippageRngState = (int)(((long)g_slippageRngState * 1103515245 + 12345) % 2147483647);
   return g_slippageRngState;
}

double ApplySlippageToPrice(const double intendedPrice, const ENUM_ORDER_TYPE orderType, int &outPips)
{
   outPips = 0;
   if(!InpEnableSlippageHarness || InpSlippageInjectionPips <= 0 || _Point <= 0.0)
      return intendedPrice;

   int maxPips = MathAbs(InpSlippageInjectionPips);
   if(maxPips <= 0)
      return intendedPrice;

   int sideBias   = (orderType == ORDER_TYPE_SELL) ? -1 : 1;
   int direction  = ((NextPseudoRandom() % 2 == 0) ? sideBias : -sideBias);
   int rawPips    = (NextPseudoRandom() % (maxPips + 1));
   int appliedAbs = MathMax(0, rawPips);
   int appliedSigned = direction * appliedAbs;
   outPips = appliedSigned;

   g_lastSlippageAppliedPts = appliedSigned;
   g_lastSlippageDirection = direction;

   return NormalizeDouble(intendedPrice + direction * _Point * appliedAbs, _Digits);
}

void LogSlippageAttempt(const string sideLabel, const string stageLabel, const double intendedPrice,
                       const double filledPrice, const int intendedPips, const int attempts,
                       const int rejects, const bool success, const int tradeRetcode)
{
   if(!InpEnableDecisionLog || !InpLogSlippageTrades)
      return;

   string outcome = success ? "SUCCESS" : "FAILED";
   int filledPips = (int)MathRound(MathAbs((filledPrice - intendedPrice) / _Point));
   string msg = StringFormat(
      "%s | %s | intention=%.5f | fill=%.5f | slip_target_pips=%d | slip_real_pips=%d | direction=%s | attempts=%d | rejects=%d | retcode=%d | state=%s",
      sideLabel, stageLabel, intendedPrice, filledPrice, intendedPips, filledPips,
      (g_lastSlippageDirection >= 0 ? "UP" : "DOWN"), attempts, rejects, tradeRetcode, outcome
   );
   DecisionLog(2, msg);
}
...
int OnInit()
{
   ...
   g_riskCurrentDay = 0;
   g_slippageRngState = MathMax(1, InpSlippageHarnessSeed);

   ApplyFilterPreset();
   UpdateRiskState(iTime(_Symbol, PERIOD_CURRENT, 0), AccountInfoDouble(ACCOUNT_EQUITY));

   Print("SniperTrendEA v8.61 鍒濆鍖栨垚鍔?| ", _Symbol, " ", EnumToString(Period()),
         " | 棰勮:", g_presetName,
...
void OnTick()
{
   ...
   if(InpDebugMode)
   {
      ...
      Comment("SniperEA v8.61 | 棰勮:", g_presetName, " | ", _Symbol, " ", EnumToString(Period()), "\n",
      ...
      if(g_pendingBuy || g_pendingSell)
      {
         ...
         else
         {
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               {
                  Print("銆愪笅褰辫繃闀?绌恒€戞斁寮?);
                  g_pendingSell = false; g_pendingBars = 0;
               }
               else if(g_requireMomentum && !HasMomentumDominance(false, 1))
               {
                  Print("銆愬姩鑳戒笉瓒?绌恒€戝疄浣撴湭寮轰簬杩?, g_momentumLookback, "鏍归槼绾匡紝鏀惧純");
                  g_pendingSell = false; g_pendingBars = 0;
               }
...
{
   string sideLabel = (type == ORDER_TYPE_BUY) ? "BUY" : "SELL";
   ...
   double intendedPrice = NormalizeDouble(price, _Digits);
   bool     sent = false;
   int      rejectCount = 0;
   for(int attempt = 0; attempt <= InpSlippageRetryLimit && !sent; attempt++)
   {
      int appliedPips = 0;
      req.price = ApplySlippageToPrice(intendedPrice, type, appliedPips);

      if(!OrderSend(req, res))
      {
         rejectCount++;
         LogSlippageAttempt(sideLabel, "OPEN", intendedPrice, req.price, appliedPips, attempt + 1, rejectCount, false, GetLastError());
         continue;
      }
      ...
      if(res.retcode == TRADE_RETCODE_DONE || res.retcode == TRADE_RETCODE_DONE_PARTIAL)
      {
         ...
         Print("開倉成功 | 票号:", res.order, " | 價格:", res.price,
               " | 手数:", lot);
         LogSlippageAttempt(sideLabel, "OPEN", intendedPrice, res.price, appliedPips, attempt + 1, rejectCount, true, (int)res.retcode);
      }
      else
      {
         rejectCount++;
         LogSlippageAttempt(sideLabel, "OPEN", intendedPrice, req.price, appliedPips, attempt + 1, rejectCount, false, (int)res.retcode);
      }
   }
...
void ClosePosition(ulong ticket)
{
   ...
   string sideLabel = (posType == POSITION_TYPE_BUY) ? "SELL_CLOSE" : "BUY_CLOSE";
   double intendedPrice = NormalizeDouble(req.price, _Digits);
   bool     closed = false;
   int      rejectCount = 0;
   for(int attempt = 0; attempt <= InpSlippageRetryLimit && !closed; attempt++)
   {
      int appliedPips = 0;
      req.price = ApplySlippageToPrice(intendedPrice, req.type, appliedPips);
      if(!OrderSend(req, res))
      {
         rejectCount++;
         LogSlippageAttempt(sideLabel, "CLOSE", intendedPrice, req.price, appliedPips, attempt + 1, rejectCount, false, GetLastError());
         continue;
      }
      ...
      if(res.retcode == TRADE_RETCODE_DONE || res.retcode == TRADE_RETCODE_DONE_PARTIAL)
      {
         ...
         LogSlippageAttempt(sideLabel, "CLOSE", intendedPrice, res.price, appliedPips, attempt + 1, rejectCount, true, (int)res.retcode);
      }
      else
      {
         rejectCount++;
         LogSlippageAttempt(sideLabel, "CLOSE", intendedPrice, req.price, appliedPips, attempt + 1, rejectCount, false, (int)res.retcode);
      }
   }
