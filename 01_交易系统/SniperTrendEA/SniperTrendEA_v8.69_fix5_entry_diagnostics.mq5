...
input string InpComment = "SniperEA_v8.69_fix5_diag";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.69_fix5_diag_loss_armed_time_gate";
...
int OnInit()
{
   ...
   if(InpUseV869RegimeGate || InpV869EntryDiagnosticLog)
   {
      ...
      if(g_pendingBuy || g_pendingSell)
      {
         ...
         else
         {
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               ...
               {
                  if(IsV869TimeClusterBlocked(true))
                  {
                     g_pendingBuy = false; g_pendingBars = 0;
                  }
                  else
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               {
                  if(IsV869TimeClusterBlocked(false))
                  {
                     g_pendingSell = false; g_pendingBars = 0;
                  }
                  else
...
bool IsV869TimeClusterBlocked(bool forBuy)
{
   ...
   {
      Print("[v8.69_fix5_diag TimeGate] blocked ",
            (forBuy ? "BUY" : "SELL"),
