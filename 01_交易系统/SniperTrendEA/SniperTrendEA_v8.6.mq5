...
void OnTick()
{
   ...
   if(InpDebugMode)
   {
      ...
      Comment("SniperEA v8.62 | 预设:", g_presetName, " | ", _Symbol, " ", EnumToString(Period()), "\n",
...
{
   //--- 自动适配券商最小止损距离 (STOPLEVEL)
   int stopLevel = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   if(stopLevel > 0)
   {
      double minSLPoints = stopLevel * _Point;
      if(type == ORDER_TYPE_SELL && sl > price && (sl - price) < minSLPoints)
         sl = price + minSLPoints;  // 空单：拉大止损
      if(type == ORDER_TYPE_BUY  && sl < price && (price - sl) < minSLPoints)
         sl = price - minSLPoints;  // 多单：拉大止损
   }
