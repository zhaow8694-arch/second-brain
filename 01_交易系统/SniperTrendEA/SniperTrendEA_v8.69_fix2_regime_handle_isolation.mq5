...
input string InpComment = "SniperEA_v8.69_fix2_regime_handles";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.69_fix2_regime_handle_isolation";
...
double GetV869IndicatorValue(int handle, int bufferIndex, int shift)
{
   ...
   if(copied <= 0)
   {
      if(InpRegimeDebugLog)
         Print("[v8.69_fix2 RegimeGate] CopyBuffer failed | handle=", handle,
...
double GetV869RegimeRiskMultiplier()
{
   ...
   if(InpRegimeDebugLog)
      Print("[v8.69_fix2 RegimeGate] multiplier=", DoubleToString(multiplier, 2),
