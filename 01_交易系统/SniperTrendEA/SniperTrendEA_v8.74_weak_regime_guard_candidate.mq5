...
input group "=== Context Scaling Candidate (v8.74) ==="
...
input bool   InpUseV874WeakRegimeGuard = false;
input int    InpV874WeakRegimeMode = 1;
input int    InpV874WeakRegimeAdxPeriod = 14;
input double InpV874WeakRegimeLowScoreThreshold = 55.0;
input double InpV874WeakRegimeMinDirectionalEdge = 8.0;
input double InpV874WeakRegimeMaxAdx = 22.0;
input double InpV874WeakRegimeRiskScale = 0.50;
input bool   InpV874WeakRegimeHardBlock = false;
input bool   InpV874WeakRegimePrint = false;
...
input string InpComment = "SniperEA_v8.74_context_scale";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.74_context_scaling_candidate";
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if(!ApplySpreadCostGovernor(lot, strong_context) || !ApplyV871ContextGate(lot, true) || !ApplyV873ContextScaling(lot, true) || !ApplyV874WeakRegimeGuard(lot, true))
                     {
                        g_pendingBuy = false; g_pendingBars = 0;
                     }
                     else
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if(!ApplySpreadCostGovernor(lot, strong_context) || !ApplyV871ContextGate(lot, false) || !ApplyV873ContextScaling(lot, false) || !ApplyV874WeakRegimeGuard(lot, false))
                     {
                        g_pendingSell = false; g_pendingBars = 0;
                     }
                     else
...
// V874_WEAK_REGIME_GUARD
// Default-off weak-regime guard candidate. It does not change signal generation.
// When enabled, it scales or blocks lot size only in low-trend / low-directional-edge regimes.
double GetV874WeakRegimeAdx()
{
   int handle = iADX(_Symbol, PERIOD_CURRENT, InpV874WeakRegimeAdxPeriod);
   if(handle == INVALID_HANDLE)
      return 100.0;

   double adx[];
   ArraySetAsSeries(adx, true);
   int copied = CopyBuffer(handle, 0, 1, 1, adx);
...
double GetV874WeakRegimeDirectionalEdge(const bool forBuy)
{
   int handle = iADX(_Symbol, PERIOD_CURRENT, InpV874WeakRegimeAdxPeriod);
   if(handle == INVALID_HANDLE)
      return 100.0;
   ...
   int copiedPlus = CopyBuffer(handle, 1, 1, 1, plusDI);
   int copiedMinus = CopyBuffer(handle, 2, 1, 1, minusDI);
   IndicatorRelease(handle);
...
double GetV874WeakRegimeScore(const bool forBuy)
{
   if(!InpUseV874WeakRegimeGuard)
      return 100.0;

   double adx = GetV874WeakRegimeAdx();
   double edge = GetV874WeakRegimeDirectionalEdge(forBuy);
   double score = 100.0;

   if(adx < InpV874WeakRegimeMaxAdx)
      score -= (InpV874WeakRegimeMaxAdx - adx) * 2.0;
   if(edge < InpV874WeakRegimeMinDirectionalEdge)
      score -= (InpV874WeakRegimeMinDirectionalEdge - edge) * 3.0;
...
double GetV874WeakRegimeMultiplier(const bool forBuy)
{
   if(!InpUseV874WeakRegimeGuard)
      return 1.0;

   double score = GetV874WeakRegimeScore(forBuy);
   if(score >= InpV874WeakRegimeLowScoreThreshold)
      return 1.0;

   if(InpV874WeakRegimeHardBlock)
      return 0.0;

   double scale = InpV874WeakRegimeRiskScale;
   if(scale < 0.0) scale = 0.0;
   if(scale > 1.0) scale = 1.0;
   return scale;
}

bool ApplyV874WeakRegimeGuard(double &lots, const bool forBuy)
{
   if(!InpUseV874WeakRegimeGuard)
      return true;
   ...
   double multiplier = GetV874WeakRegimeMultiplier(forBuy);
   if(multiplier <= 0.0)
   {
      if(InpV874WeakRegimePrint)
         Print("V874_WEAK_REGIME_GUARD hard block | direction=", (forBuy ? "buy" : "sell"),
               " | score=", DoubleToString(GetV874WeakRegimeScore(forBuy), 2));
      lots = 0.0;
      return false;
   }
   ...
   if(InpV874WeakRegimePrint && MathAbs(lots - originalLots) > 0.0000001)
      Print("V874_WEAK_REGIME_GUARD scaled | direction=", (forBuy ? "buy" : "sell"),
            " | score=", DoubleToString(GetV874WeakRegimeScore(forBuy), 2),
            " | lot=", DoubleToString(originalLots, 2), "->", DoubleToString(lots, 2));
