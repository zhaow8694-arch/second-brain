...
//+------------------------------------------------------------------+
#property copyright "SniperTrendEA v8.76 - v8.6 Spine + Selective Context Overlay"
...
input group "=== Context Scaling Candidate (v8.73) ==="
...
input group "=== v8.76 Selective Context Overlay ==="
input bool   InpUseV876ContextOverlay       = true;
input ENUM_V876_OVERLAY_MODE InpV876OverlayMode = V876_OVERLAY_OBSERVE;
input bool   InpV876ProfitSpineVeto         = true;
input bool   InpV876ExecutionCostGuard      = true;
input bool   InpV876DecisionLog             = true;
input string InpV876DecisionLogFile         = "v876_context_overlay_decisions.csv";
input bool   InpV876DecisionLogCommon       = true;
input bool   InpV876ContextDebug            = false;
input double InpV876ProfitSpineMinScore     = 70.0;
input double InpV876LossCompressionMinScore = 55.0;
input double InpV876WeakClusterMaxScore     = 54.9;
...
input string InpComment = "SniperEA_v8.73_context_scale";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.76_context_overlay_candidate";
const string EA_PARAMETER_PROFILE = "v8.76_context_overlay_observe_first";
const string EA_RECOMMENDED_SET   = "v8.76_observe_default.set";
...
void OnDeinit(const int reason)
{
   ...
   CloseV876DecisionLogFile();if(g_v869DiagFileHandle != INVALID_HANDLE)
   {
      FileFlush(g_v869DiagFileHandle);
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if(!ApplySpreadCostGovernor(lot, strong_context) || !ApplyV871ContextGate(lot, true) || !ApplyV873ContextScaling(lot, true))
                     {
                        g_pendingBuy = false; g_pendingBars = 0;
                     }
                     else
                     {
                        SV876Decision v876BuyDecision;
                        BuildV876Decision(v876BuyDecision, true, "base_buy_signal", bodyRatio, lot, atr1, dangerCandle);
                        if(!ApplyV876OverlayDecision(v876BuyDecision))
                        {
                           g_pendingBuy = false; g_pendingBars = 0;
                        }
                        else
                        {
                           lot = v876BuyDecision.finalLot;
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if(!ApplySpreadCostGovernor(lot, strong_context) || !ApplyV871ContextGate(lot, false) || !ApplyV873ContextScaling(lot, false))
                     {
                        g_pendingSell = false; g_pendingBars = 0;
                     }
                     else
                     {
                        SV876Decision v876SellDecision;
                        BuildV876Decision(v876SellDecision, false, "base_sell_signal", bodyRatio, lot, atr1, dangerCandle);
                        if(!ApplyV876OverlayDecision(v876SellDecision))
                        {
                           g_pendingSell = false; g_pendingBars = 0;
                        }
                        else
                        {
                           lot = v876SellDecision.finalLot;
...
double GetV869DiEdgeHourLotScale(bool forBuy)
{
   ...
   if(InpUseV876ContextOverlay) return 1.0;if(!InpUseV869DiEdgeHourScale)
      return 1.0;
...
{
   if(InpUseV876ContextOverlay) return 1.0;
...
bool IsV869DiEdgeHourBlocked(bool forBuy)
{
   ...
   if(InpUseV876ContextOverlay && InpV876OverlayMode != V876_OVERLAY_VETO_AWARE) return false;if(!InpUseV869DiEdgeHourGate)
      return false;
...
{
   if(InpUseV876ContextOverlay && InpV876OverlayMode != V876_OVERLAY_VETO_AWARE) return false;
...
bool ApplySpreadCostGovernor(double &lots, const bool strong_context)
{
   ...
   if(InpUseV876ContextOverlay) return true;if(InpSpreadGovernorMode <= 0)
      return true;
...
{
   if(InpUseV876ContextOverlay) return true;
...
//+------------------------------------------------------------------+
//| v8.76 selective context overlay helpers                           |
//+------------------------------------------------------------------+
...
void ResetV876Decision(SV876Decision &d)
{
   ...
   d.structureScore = 0.0;
   d.breakoutDistanceATR = 0.0;
   d.bodyRatio = 0.0;
...
bool OpenV876DecisionLogFile()
{
   ...
   FileWrite(g_v876DecisionLogHandle,
             ...
             "base_signal", "base_signal_reason", "structure_score", "breakout_distance_atr",
             "body_ratio", "opposite_shadow_ratio", "wick_to_body", "wick_conflict",
             "momentum_dominance", "di_edge", "adx", "spread_points",
             "context_class", "profit_spine_veto", "cost_state",
...
void WriteV876DecisionLog(const SV876Decision &d)
{
   if(!InpV876DecisionLog || g_v876DecisionLogHandle == INVALID_HANDLE)
      return;
   datetime barTime = iTime(_Symbol, PERIOD_CURRENT, 1);
   FileWrite(g_v876DecisionLogHandle,
             TimeToString(TimeCurrent(), TIME_DATE | TIME_SECONDS), _Symbol, EnumToString(PERIOD_CURRENT), TimeToString(barTime, TIME_DATE | TIME_SECONDS),
             d.forBuy ? "BUY" : "SELL", d.baseSignal ? "true" : "false", d.baseSignalReason,
             DoubleToString(d.structureScore, 2), DoubleToString(d.breakoutDistanceATR, 3), DoubleToString(d.bodyRatio, 3), DoubleToString(d.oppositeShadowRatio, 3),
             DoubleToString(d.wickToBody, 3), d.wickConflict ? "true" : "false", d.momentumDominance ? "true" : "false",
             DoubleToString(d.directionalEdge, 2), DoubleToString(d.adx, 2), DoubleToString(d.spreadPoints, 1),
             V876ContextClassToString(d.contextClass), d.profitSpineVeto ? "true" : "false", V876CostStateToString(d.costState),
             DoubleToString(d.contextLotScale, 3), DoubleToString(d.spreadLotScale, 3), DoubleToString(d.baseLot, 2), DoubleToString(d.finalLot, 2),
             d.finalAction, d.rejectReason, V876OverlayModeToString());
   FileFlush(g_v876DecisionLogHandle);
}

double GetV876DirectionalEdge(const bool forBuy)
{
   double plusDi = GetV869IndicatorValue(g_v869AdxHandle, 1, 1);
   double minusDi = GetV869IndicatorValue(g_v869AdxHandle, 2, 1);
   if(plusDi == EMPTY_VALUE || minusDi == EMPTY_VALUE)
      return 0.0;
   return forBuy ? (plusDi - minusDi) : (minusDi - plusDi);
}

ENUM_V876_CONTEXT_CLASS ClassifyV876Context(const SV876Decision &d)
{
   bool strongStructure = (d.structureScore >= InpV876ProfitSpineMinScore);
   bool cleanCandle = (!d.wickConflict && d.bodyRatio >= g_bodyRatio);
   bool strongMomentum = d.momentumDominance;
   bool strongDirectionalEdge = (d.directionalEdge >= InpV869ContextMinDiEdge);
   bool lowAdx = (d.adx > 0.0 && d.adx < InpV869ContextMaxAdx);
   bool weakStructure = (d.structureScore > 0.0 && d.structureScore <= InpV876WeakClusterMaxScore);
   bool midStructure = (d.structureScore >= InpV876LossCompressionMinScore && d.structureScore < InpV876ProfitSpineMinScore);
   if(strongStructure && cleanCandle && strongMomentum && strongDirectionalEdge) return V876_CONTEXT_PROFIT_SPINE;
   if(midStructure && (lowAdx || !strongDirectionalEdge)) return V876_CONTEXT_LOSS_COMPRESSION;
   if(weakStructure || d.wickConflict) return V876_CONTEXT_WEAK_CLUSTER;
   if(midStructure) return V876_CONTEXT_MIXED;
   return V876_CONTEXT_UNKNOWN;
}

void BuildV876Decision(SV876Decision &d, const bool forBuy, const string baseReason, const double bodyRatio, const double rawLot, const double atr, const bool dangerousCandle)
{
   ResetV876Decision(d);
   d.baseSignal = true;
   d.forBuy = forBuy;
   ...
   d.baseLot = rawLot;
   d.finalLot = rawLot;
   d.oppositeShadowRatio = forBuy ? GetUpperShadowRatio(1) : GetLowerShadowRatio(1);
   ...
   d.adx = GetV869ADXValue(InpADXPeriod, 1);
   d.spreadPoints = GetCurrentSpreadPoints();
   STrendlineInfo info;
   double quality = 1.0;
   if(GetStructureQualityFactor(forBuy, atr, dangerousCandle, info, quality))
   {
      d.structureScore = info.score;
      d.breakoutDistanceATR = info.breakoutDistanceATR;
   }
   d.contextClass = ClassifyV876Context(d);
   d.finalAction = "open_full";
}

double ResolveV876ContextScale(SV876Decision &d)
{
   d.profitSpineVeto = false;
   d.contextLotScale = 1.0;
   if(!InpUseV876ContextOverlay || InpV876OverlayMode == V876_OVERLAY_OFF) return 1.0;
   if(d.contextClass == V876_CONTEXT_PROFIT_SPINE)
   {
      if(InpV876ProfitSpineVeto) d.profitSpineVeto = true;
      return 1.0;
   }
   if(InpV876OverlayMode == V876_OVERLAY_OBSERVE) return 1.0;
   if(d.contextClass == V876_CONTEXT_LOSS_COMPRESSION) d.contextLotScale = MathMax(0.01, MathMin(1.0, InpV873MidScoreLotScale));
   else if(d.contextClass == V876_CONTEXT_WEAK_CLUSTER) d.contextLotScale = MathMax(0.01, MathMin(1.0, InpV876WeakContextScale));
   else if(d.contextClass == V876_CONTEXT_MIXED) d.contextLotScale = MathMax(0.01, MathMin(1.0, InpV876MixedContextScale));
   else d.contextLotScale = MathMax(0.01, MathMin(1.0, InpV876UnknownContextScale));
   return d.contextLotScale;
}

bool ApplyV876ContextAction(SV876Decision &d)
{
   double scale = ResolveV876ContextScale(d);
   if(InpV876OverlayMode == V876_OVERLAY_VETO_AWARE && d.contextClass == V876_CONTEXT_WEAK_CLUSTER && scale <= 0.0 && !d.profitSpineVeto)
   {
   ...
   d.finalLot = d.finalLot * scale;
   if(scale < 1.0) d.finalAction = "open_scaled";
   return true;
}

ENUM_V876_COST_STATE ClassifyV876CostState(const double spreadPoints)
{
   if(spreadPoints >= InpSpreadHardBlockPoints) return V876_COST_HARD_BLOCK;
   if(spreadPoints >= InpSpreadWeakStartPoints) return V876_COST_WEAK;
   if(spreadPoints > InpSpreadNormalMaxPoints) return V876_COST_WATCH;
...
bool ApplyV876ExecutionCostGuard(SV876Decision &d)
{
   ...
   d.spreadLotScale = 1.0;
   if(!InpV876ExecutionCostGuard || !InpUseV876ContextOverlay || InpV876OverlayMode == V876_OVERLAY_OFF) return true;
   if(InpV876OverlayMode == V876_OVERLAY_OBSERVE) return true;
   if(d.costState == V876_COST_HARD_BLOCK)
   {
      if(InpV876OverlayMode == V876_OVERLAY_VETO_AWARE && !d.profitSpineVeto)
      {
      ...
      d.spreadLotScale = MathMax(0.01, MathMin(1.0, InpSpreadWeakLotMultiplier));
   }
   else if(d.costState == V876_COST_WEAK)
   {
      bool strongContext = (d.contextClass == V876_CONTEXT_PROFIT_SPINE);
      if(InpV876OverlayMode == V876_OVERLAY_VETO_AWARE && InpSpreadRequireStrongContextInWeakZone && !strongContext && !d.profitSpineVeto)
      {
         d.executionAllowsTrade = false;
         d.finalAction = "reject_execution_cost";
         d.rejectReason = "v876_weak_spread_without_strong_context";
         return false;
      }
      d.spreadLotScale = MathMax(0.01, MathMin(1.0, InpSpreadWeakLotMultiplier));
   }
   ...
   {
      d.spreadLotScale = MathMax(0.01, MathMin(1.0, InpSpreadWatchLotMultiplier));
   }
   d.finalLot = d.finalLot * d.spreadLotScale;
   if(d.spreadLotScale < 1.0 && d.finalAction == "open_full") d.finalAction = "open_scaled_cost";
   return true;
}

bool ApplyV876OverlayDecision(SV876Decision &d)
{
   if(!InpUseV876ContextOverlay || InpV876OverlayMode == V876_OVERLAY_OFF)
   {
      d.finalLot = d.baseLot;
      d.finalAction = "open_full_overlay_off";
      WriteV876DecisionLog(d);
      return (d.finalLot > 0.0);
   }
   if(!ApplyV876ContextAction(d)) { WriteV876DecisionLog(d); return false; }
   if(!ApplyV876ExecutionCostGuard(d)) { WriteV876DecisionLog(d); return false; }
   if(InpV876OverlayMode == V876_OVERLAY_OBSERVE)
   {
      d.finalLot = d.baseLot;
      d.finalAction = "observe_open_full";
   }
   if(d.finalLot <= 0.0)
   {
      d.contextAllowsTrade = false;
      d.finalAction = "reject_zero_lot";
