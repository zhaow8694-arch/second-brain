...
input string InpComment = "SniperEA_v8.69_fix5c";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.69_fix5c_loss_armed_time_gate";
...
int OnInit()
{
   ...
   if(InpUseV869RegimeGate || InpV869EntryDiagnosticLog || InpUseV869DiEdgeHourGate || InpUseV869DiEdgeHourScale)
   {
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     double rawLot = CalculateLotSize(ep - sl) * structureFactor * GetV869DiEdgeHourLotScale(true);
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     double rawLot = CalculateLotSize(sl - ep) * structureFactor * GetV869DiEdgeHourLotScale(false);
                     double lot = (rawLot > 0.0 ? AdjustLotSize(rawLot) : 0.0);
...
double GetV869DiEdgeHourLotScale(bool forBuy)
{
   ...
   {
      Print("[v8.69_fix5c DiEdgeScale] scaled ",
            (forBuy ? "BUY" : "SELL"),
            " hour=", dt.hour,
...
bool IsV869DiEdgeHourBlocked(bool forBuy)
{
   ...
   {
      Print("[v8.69_fix5c DiEdgeGate] blocked ",
            (forBuy ? "BUY" : "SELL"),
            " hour=", dt.hour,
            " edge=", DoubleToString(directionalEdge, 2),
            " minEdge=", DoubleToString(InpV869MinDirectionalDiEdge, 2),
...
bool IsV869TimeClusterBlocked(bool forBuy)
{
   ...
   {
      Print("[v8.69_fix5c TimeGate] blocked ",
            (forBuy ? "BUY" : "SELL"),
