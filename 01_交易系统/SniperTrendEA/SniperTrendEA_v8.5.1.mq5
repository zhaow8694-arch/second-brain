...
//|       —— 对应文章《危险的K线》                                     |
//|       ✦ v8trae增强：增加突兀性检测，必须前一根较小+当前突然暴增     |
//|                                                                  |
...
//|                                                                  |
//|  【6】v8trae新增：动能递增检查 (RequireMomentumShift)：            |
//|       确认K线实体必须 ≥ 前一根实体（d点动能 > c点动能），          |
//|       防止假反转信号（如结构观察中的b点假突破）。                  |
//|       —— 对应文章《市场结构观察 #26-2-2》《突破质量的深度评估》     |
//+------------------------------------------------------------------+
#property copyright "SniperTrendEA v8.5trae - Wyckoff + Evil MACD + Z-Wei Philosophy"
#property version   "8.50trae"
#property strict
...
input double InpMA200BufferATR = 0.3;       // 【v8trae】MA200缓冲区（ATR倍数，默认0.3适应震荡）

input group "=== 入场质量过滤（v8trae 增强 Z-Wei 哲学）==="
input double InpBodyRatio          = 0.60;  // K线实体占比阈值（≥60%）
input double InpMaxCandleATR       = 2.5;   // 【v8.5】K线最大振幅（ATR倍数），防追高耗竭
input double InpDangerSuddenRatio  = 2.0;   // 【v8trae新增】危险K线突兀倍数（当前振幅/前根振幅>此值）
input double InpMaxOppositeShadow  = 0.20;  // 【v8.5】反向影线最大占比
input bool   InpRequireFollowThrough = false;// 【v8.5】是否要求收盘创近N根K线极值
input int    InpFollowThroughBars  = 3;     // 【v8.5】跟随确认回看K线数
input int    InpConfirmBars        = 4;     // 翻转后等待确认的最大K线数
input bool   InpRequireMACDDir     = false; // 是否要求MACD主线方向一致
input bool   InpRequireMomentumShift = false;// 【v8trae新增】是否要求确认K线动能递增（d点>c点）
...
input string InpComment        = "SniperEA_v8.5";
...
int OnInit()
{
   ...
   Print("SniperTrendEA v8.5trae 初始化成功 | ", _Symbol, " ", EnumToString(Period()),
         " | 危险K线阈值:", InpMaxCandleATR, "×ATR (突兀:", InpDangerSuddenRatio, "×)",
         " | 反向影线限制:", DoubleToString(InpMaxOppositeShadow * 100, 0), "%",
         " | 跟随确认:", InpRequireFollowThrough ? "ON" : "OFF",
         " | 动能递增:", InpRequireMomentumShift ? "ON" : "OFF",
...
void OnTick()
{
   ...
   if(InpDebugMode)
   {
      ...
      Comment("SniperEA v8.5trae 哲学融合版 | ", _Symbol, " ", EnumToString(Period()), "\n",
      ...
      if(g_pendingBuy || g_pendingSell)
      {
         ...
         else
         {
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               ...
               {
                  Print("【动能未递增-多】当前实体未>前一根，疑似假反转(d点动能不足)，放弃");
                  g_pendingBuy = false; g_pendingBars = 0;
               }
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               {
                  Print("【动能未递增-空】当前实体未>前一根，疑似假反转(d点动能不足)，放弃");
                  g_pendingSell = false; g_pendingBars = 0;
               }
...
//+------------------------------------------------------------------+
//| 【v8trae新增】动能递增检查（d点实体 ≥ c点实体）                    |
//| 对应《市场结构观察》：d点动能明显大于c点才是真正反转               |
//+------------------------------------------------------------------+
