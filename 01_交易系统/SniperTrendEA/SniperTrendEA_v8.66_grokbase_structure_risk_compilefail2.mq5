...
input ENUM_FILTER_PRESET InpFilterPreset = FILTER_AGGRESSIVE; // 过滤强度（对�?grok8.6 锚点默认=2�?
input group "=== MACD 参数 ==="
...
input double InpMA200BufferATR = 0.2;       // �?FILTER_CUSTOM 时生�?
input group "=== 入场质量过滤（v8.5，仅 FILTER_CUSTOM 时生效）==="
input double InpBodyRatio          = 0.55;  // K线实体占比（均衡默认 0.55�?input double InpMaxCandleATR       = 3.0;   // 危险K线阈值（均衡默认 3.0�?input double InpMaxOppositeShadow  = 0.30;  // 反向影线上限（均衡默�?30%�?input bool   InpRequireFollowThrough = false;
input int    InpFollowThroughBars  = 3;
input int    InpConfirmBars        = 3;     // 翻转确认等待K线（均衡默认 3�?input bool   InpRequireMACDDir     = false;
...
input group "=== Structure Score Soft Filter (v8.66) ==="
input bool   InpUseStructureScore       = false;
...
input bool   InpUseIgnitionExit     = true;   // 点火失败快速平�?input int    InpIgnitionMaxBars     = 3;      // 入场后观察K线数
input double InpIgnitionEngulfRatio = 0.85;   // 反向实体/入场实体 触发阈�?input double InpIgnitionMaxLossATR  = 1.0;    // 仅在此ATR亏损范围内执行点火止�?
input group "=== 交易设置 ==="
...
int OnInit()
{
   ...
   Print("SniperTrendEA v8.61 初始化成�?| ", _Symbol, " ", EnumToString(Period()),
         " | 预设:", g_presetName,
...
void OnTick()
{
   ...
   if(InpDebugMode)
   {
      ...
      Comment("SniperEA v8.61 | 预设:", g_presetName, " | ", _Symbol, " ", EnumToString(Period()), "\n",
              ...
              " | �?�?", DoubleToString(GetWickToBodyRatio(1), 2), "\n",
      ...
      if(g_pendingBuy || g_pendingSell)
      {
         ...
         else
         {
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               if(dangerCandle)
               {
                  Print("【危险K�?多】振�?", g_maxCandleATR, "×ATR，放�?);
                  g_pendingBuy = false; g_pendingBars = 0;
               }
               else if(wickConflict)
               {
                  Print("【双向博�?多】总影�?实体=", DoubleToString(GetWickToBodyRatio(1), 2),
                        " > ", g_maxWickToBody, "，放�?);
                  g_pendingBuy = false; g_pendingBars = 0;
               }
               ...
               else
               {
                  ...
                  Print("【开多】实�?", DoubleToString(bodyRatio * 100, 1), "%",
                        " | �?�?", DoubleToString(GetWickToBodyRatio(1), 2),
                        " | EP:", ep, " SL:", sl, " Lot:", lot, " | StructQ:", DoubleToString(structureFactor, 3));
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               {
                  Print("【双向博�?空】总影�?实体=", DoubleToString(GetWickToBodyRatio(1), 2),
                        " > ", g_maxWickToBody, "，放�?);
                  g_pendingSell = false; g_pendingBars = 0;
               }
               ...
               else
               {
                  ...
                  double lot = (rawLot > 0.0 ? AdjustLotSize(rawLot) : 0.0);
                  Print("【开空】实�?", DoubleToString(bodyRatio * 100, 1), "%",
                        " | �?�?", DoubleToString(GetWickToBodyRatio(1), 2),
                        " | EP:", ep, " SL:", sl, " Lot:", lot, " | StructQ:", DoubleToString(structureFactor, 3));
...
bool OpenPosition(ENUM_ORDER_TYPE type, double price, double sl, double lot)
{
   ...
   {
      Print("开仓失�?| 错误:", GetLastError(), " | 类型:", EnumToString(type), " | 手数:", lot);
      return false;
   }

   g_entryBarTime  = iTime(_Symbol, PERIOD_CURRENT, 1);
   g_entryBodySize = GetCandleBody(1);
   Print("开仓成�?| 票号:", res.order, " | 价格:", res.price,
