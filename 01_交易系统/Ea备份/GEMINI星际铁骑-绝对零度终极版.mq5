...
void OnDeinit(const int reason) { Print("⚠️ 机甲回港，代码: ", reason); }
...
{
   Sleep(500); double bal_after = accInfo.Balance(); double delta = bal_after - bal_before;
   double dTotal = bal_after - DailyStartBalance;
   string emoji = (delta >= 0) ? "💰 净利: +" : "💀 战损: -";
   string dEmoji = (dTotal >= 0) ? "📈 今日累计: +" : "📉 今日累计: -";
   // 🎯【最终修复】：已确保 dEmoji 和 dTotal 变量名与下方字符串完美对齐
