...
{
   if(AccountInfoDouble(ACCOUNT_MARGIN_FREE) / AccountInfoDouble(ACCOUNT_BALANCE) < 0.35) return;
   ...
   // 保证金安全检查
   double margin = 0;
   if(OrderCalcMargin(type, _Symbol, lot, price, margin) && margin > AccountInfoDouble(ACCOUNT_MARGIN_FREE) * 0.75)
      lot = MathFloor((AccountInfoDouble(ACCOUNT_MARGIN_FREE) * 0.75 / (margin/lot)) / step) * step;
