...
input ENUM_ORDER_TYPE_FILLING InpForceFillingMode = ORDER_FILLING_AUTOMATIC; // 强制成交模式 (回测兼容增强)
...
int OnInit()
{
   ...
   // [Manus加固] 自适应成交模式探测 - 彻底解决 GetTypeFilling 兼容性问题
   if(InpForceFillingMode != ORDER_FILLING_AUTOMATIC)
   {
      m_trade.SetTypeFilling(InpForceFillingMode);
      Print("Manus v1.1 加固版初始化成功. 强制成交模式: ", EnumToString(InpForceFillingMode));
   }
   else
   {
      uint filling_mode = (uint)SymbolInfoInteger(_Symbol, SYMBOL_FILLING_MODE);
      ...
      Print("Manus v1.1 加固版初始化成功. 模式自适应已完成.");
   }
