...
public:
   ...
   void UpdateState(SStateResult &detection)
   {
      ...
      double effectiveConfidenceReq = m_confidenceReq;
      int    effectiveMinBars       = m_minSwitchBars;

      if(m_currentState == MARKET_STATE_TRENDING && detection.state == MARKET_STATE_RANGING)
      {
         effectiveConfidenceReq += 5.0;
         effectiveMinBars += 2;
      }

      if(m_pendingSwitchCount >= effectiveMinBars &&
         detection.confidence >= effectiveConfidenceReq &&
         detection.state != MARKET_STATE_UNKNOWN)
      {
