...
public:
   ...
   STradeSignal GenerateSignal()
   {
      ...
      double fast[3], slow[3], atr[1], adx[1], diPlus[1], diMinus[1];

      if(CopyBuffer(m_fastEMAHandle, 0, 1, 3, fast) < 3 ||
         CopyBuffer(m_slowEMAHandle, 0, 1, 3, slow) < 3 ||
         CopyBuffer(m_atrHandle, 0, 1, 1, atr) < 1 ||
         CopyBuffer(m_adxHandle, 0, 1, 1, adx) < 1 ||
         CopyBuffer(m_adxHandle, 1, 1, 1, diPlus) < 1 ||
         CopyBuffer(m_adxHandle, 2, 1, 1, diMinus) < 1)
         return signal;
      ...
      bool emaCrossUp   = (fast[1] <= slow[1] && fast[0] > slow[0]);
      bool emaCrossDown = (fast[1] >= slow[1] && fast[0] < slow[0]);
      bool pullbackBuy  = (fast[0] > slow[0] && diPlus[0] > diMinus[0] &&
                           bidPrice <= fast[0] && bidPrice > slow[0] &&
                           fast[1] > slow[1]);
      bool pullbackSell = (fast[0] < slow[0] && diMinus[0] > diPlus[0] &&
                           askPrice >= fast[0] && askPrice < slow[0] &&
                           fast[1] < slow[1]);

      bool buySignal  = (emaCrossUp && diPlus[0] > diMinus[0]) || pullbackBuy;
      bool sellSignal = (emaCrossDown && diMinus[0] > diPlus[0]) || pullbackSell;
