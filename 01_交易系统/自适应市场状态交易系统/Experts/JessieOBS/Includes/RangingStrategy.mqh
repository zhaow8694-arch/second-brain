...
public:
   ...
   STradeSignal GenerateSignal()
   {
      ...
      if(CopyBuffer(m_bbHandle, 1, 1, 1, upper) < 1 ||
         CopyBuffer(m_bbHandle, 0, 1, 1, middle) < 1 ||
         CopyBuffer(m_bbHandle, 2, 1, 1, lower) < 1 ||
         CopyBuffer(m_rsiHandle, 0, 1, 1, rsi) < 1 ||
         CopyBuffer(m_atrHandle, 0, 1, 1, atr) < 1 ||
         CopyClose(m_symbol, m_timeframe, 1, 1, closePrice) < 1)
         return signal;
      ...
      if(atrVal > 0 && bbWidth > atrVal * 4.0) return signal;

      bool buySignal  = false;
      bool sellSignal = false;

      if(closePrice[0] <= lower[0] && rsi[0] <= m_rsiOS)
         buySignal = true;
      else if(closePrice[0] >= upper[0] && rsi[0] >= m_rsiOB)
         sellSignal = true;
      else if(closePrice[0] <= middle[0] - atrVal * 0.35 && rsi[0] < 42)
         buySignal = true;
      else if(closePrice[0] >= middle[0] + atrVal * 0.35 && rsi[0] > 58)
         sellSignal = true;
