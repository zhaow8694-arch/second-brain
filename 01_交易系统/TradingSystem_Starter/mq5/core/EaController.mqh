...
private:
   ...
   string DateTimeValueToText(const datetime value)
   {
      if(value <= 0)
      {
         return "none";
   ...
   string BuildLastNTicksMetrics()
   {
      return "total_ticks=" + IntegerToString(totalTicks)
             + ",last_tick_timestamp=" + DateTimeValueToText(lastTickTimestamp);
   }
...
public:
   ...
   void OnTick()
   {
      ...
      if(InpObservabilityLogOnTick && InpLogEveryNTicks > 0 && (totalTicks % InpLogEveryNTicks) == 0)
      {
