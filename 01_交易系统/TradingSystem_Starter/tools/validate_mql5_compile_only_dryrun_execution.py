#!/usr/bin/env python3
"""Read-only MQL5 compile-only dry-run execution validator.

The validator emits a stdout-only simulation result for a future compile-only
execution candidate. It never executes MetaEditor, MT5, terminal64.exe, or MQL5
compile commands.
"""
...
TASK300_DOC = ROOT_DIR / "docs" / "V060_TASK_300_MQL5_COMPILE_ONLY_DRYRUN_SIMULATION.md"

TASK300_REQUIRED_KEYWORDS = (
...
def collect_boundary_issues(root_dir: Path) -> list[str]:
    ...
    docs = (
        ...
        ("TASK-300 compile-only dry-run execution doc", root_dir / "docs" / TASK300_DOC.name),
    )
    ...
    task300_path = root_dir / "docs" / TASK300_DOC.name
    if not task300_path.exists():
        return issues

    text = read_text(task300_path)
    for keyword in TASK300_REQUIRED_KEYWORDS:
        if keyword not in text:
            issues.append(f"TASK-300 dry-run execution doc missing keyword: {keyword}")
    return issues
...
def print_failure(
    ...
    *,
    ...
    print("MQL5 compile-only dry-run execution validation failed")
    print("Issues:")
    for issue in issues:
        print(f"- {issue}")
    print("mql5_compile_only_dryrun_execution=false")
    print("dry_run_execution_only=true")
    ...
    print("candidate_compile_command_generated=true")
    print("candidate_compile_command_executed=false")
    print("metaeditor_executed=false")
    print("mql5_compile_executed=false")
    print("mt5_run=false")
    print("strategy_tester=false")
    print("backtest=false")
    ...
    print("TASK_301_must_not_be_entered_directly=true")
    print(SAFETY_NOTICE)


def print_success(root_dir: Path, inventory_count: int) -> None:
    print("MQL5 compile-only dry-run execution validation passed")
    print("mql5_compile_only_dryrun_execution=true")
    print("dry_run_execution_only=true")
    print("stdout_only_simulation=true")
    print("artifact_quarantine_enforced=true")
    print("future_compile_only_task_requires_GPT_authorization=true")
