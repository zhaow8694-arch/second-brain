#!/usr/bin/env python3
"""Read-only MQL5 compile-only execution boundary validator.

This validator checks the TASK-297 boundary and repository safety state. It
never executes MetaEditor, MT5, terminal64.exe, or MQL5 compile commands.
"""
...
def collect_boundary_issues(root_dir: Path) -> list[str]:
    ...
    task297_path = root_dir / "docs" / TASK297_DOC.name
    if not task297_path.exists():
        return issues

    text = read_text(task297_path)
    for keyword in TASK297_REQUIRED_KEYWORDS:
        if keyword not in text:
            issues.append(f"TASK-297 boundary doc missing keyword: {keyword}")
    return issues
...
def print_failure(
    ...
    *,
    ...
    print("MQL5 compile-only execution boundary validation failed")
    print("Issues:")
    for issue in issues:
        print(f"- {issue}")
    print("mql5_compile_only_execution_boundary=false")
    print("compile_only_task=true")
    print("future_compile_only_candidate=true")
    print("requires_gpt_explicit_authorization=true")
    print("artifact_quarantine_checked=true")
...
def print_success(root_dir: Path, inventory_count: int) -> None:
    print("MQL5 compile-only execution boundary validation passed")
    print("mql5_compile_only_execution_boundary=true")
    print("compile_only_task=true")
    print("future_compile_only_candidate=true")
    print("requires_gpt_explicit_authorization=true")
    print("artifact_quarantine_checked=true")
    ...
    print("future_TASK_298_requires_GPT_authorization=true")
    print("future_TASK_298_must_not_be_entered_directly=true")
    print(SAFETY_NOTICE)
