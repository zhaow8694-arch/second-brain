#!/usr/bin/env python3
"""Validate TASK-307 MQL5 compile diagnostic artifact classification.
...
def collect_doc_issues() -> list[str]:
    ...
    text = read_text(TASK307_DOC_PATH)
    return [
        "docs/V060_TASK_307_MQL5_COMPILE_DIAGNOSTIC_ARTIFACT_CLASSIFICATION.md "
...
def main() -> int:
    ...
    if issues:
        print("MQL5 compile diagnostic artifact classification validation failed")
        print("Issues:")
        for issue in issues:
            print(f"- {issue}")
        return 1

    print("MQL5 compile diagnostic artifact classification validation passed")
    print("mql5_compile_diagnostic_artifact_classification=true")
    print("diagnostic_artifact_classification_only=true")
