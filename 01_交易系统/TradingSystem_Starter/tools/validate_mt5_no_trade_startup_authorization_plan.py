#!/usr/bin/env python3
"""Validate TASK-318 MT5 no-trade startup authorization planning boundary.
...
def collect_doc_issues() -> list[str]:
    ...
    if not TASK318_DOC_PATH.exists():
        issues.append(
    ...
    task318_text = read_text(TASK318_DOC_PATH)
    issues.extend(
        "docs/V060_TASK_318_MT5_NO_TRADE_STARTUP_AUTHORIZATION_PLAN.md "
        f"missing keyword: {keyword}"
        for keyword in REQUIRED_TASK318_KEYWORDS
        if keyword not in task318_text
    )
...
def main() -> int:
    ...
    if issues:
        print("MT5 no-trade startup authorization plan validation failed")
        print("Issues:")
        for issue in issues:
            print(f"- {issue}")
        return 1

    print("MT5 no-trade startup authorization plan validation passed")
    print("mt5_no_trade_startup_authorization_plan=true")
    ...
    print("future_task_319_requires_gpt_boundary=true")
