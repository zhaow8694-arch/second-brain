#!/usr/bin/env python3
"""Validate TASK-309 MQL5 compile-only success reclassification boundary.
...
def collect_doc_issues() -> list[str]:
    ...
    for path, rel_path in required_docs:
        if not path.exists():
            issues.append(f"missing required docs file: {rel_path}")

    if not TASK309_DOC_PATH.exists():
        return issues

    text = read_text(TASK309_DOC_PATH)
    issues.extend(
        "docs/V060_TASK_309_MQL5_COMPILE_ONLY_SUCCESS_RECLASSIFICATION_BOUNDARY.md "
...
def main() -> int:
    ...
    if issues:
        print("MQL5 compile success reclassification boundary validation failed")
        print("Issues:")
        for issue in issues:
            print(f"- {issue}")
        return 1

    print("MQL5 compile success reclassification boundary validation passed")
    print("mql5_compile_success_reclassification_boundary=true")
    print("success_reclassification_boundary_only=true")
    ...
    print("future_task_310_requires_gpt_boundary=true")
