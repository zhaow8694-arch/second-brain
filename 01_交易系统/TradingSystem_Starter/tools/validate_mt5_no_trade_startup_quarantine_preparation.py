#!/usr/bin/env python3
"""Validate TASK-315 MT5 no-trade startup quarantine preparation boundary.
...
ALLOWED_TERMINAL_DATA_DIR_PATTERNS = (
    "mq5/config",
)
...
def collect_doc_issues() -> list[str]:
    ...
    if not TASK315_DOC_PATH.exists():
        issues.append(
    ...
    issues.extend(
        ...
        for keyword in REQUIRED_TASK315_KEYWORDS
        if keyword not in task315_text
    )
    ...
    if TASK312_DOC_PATH.exists():
        ...
        for keyword in ("compile_success_scope=compile-only-diagnostic",):
            if keyword not in task312_text:
                issues.append(
...
def collect_repo_artifact_issues() -> tuple[list[str], bool, bool, bool, bool]:
    issues: list[str] = []
    ...
    return issues, repo_ex5_artifacts, repo_compile_logs, repo_terminal_data_directory, repo_startup_logs
...
def main() -> int:
    ...
    if issues:
        print("MT5 no-trade startup quarantine preparation validation failed")
        print("Issues:")
        for issue in issues:
            print(f"- {issue}")
        return 1

    print("MT5 no-trade startup quarantine preparation validation passed")
    print("mt5_no_trade_startup_quarantine_preparation=true")
    print("startup_quarantine_preparation_only=true")
    ...
    print("future_task_316_requires_gpt_boundary=true")
