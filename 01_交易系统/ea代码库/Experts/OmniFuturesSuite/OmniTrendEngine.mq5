...
input ulong  InpMagicNumber = 26061602;
...
input string InpPushPrefix = "OmniTrend";
...
int OnInit()
{
   ...
   g_notify.Info("Starting OmniTrendEngine. " + g_account.Summary());
...
void OnDeinit(const int reason)
{
   ...
   for(int i = 0; i < ArraySize(g_market); i++)
      g_market[i].Deinit();
   g_notify.Info("OmniTrendEngine stopped. reason=" + IntegerToString(reason));
}
