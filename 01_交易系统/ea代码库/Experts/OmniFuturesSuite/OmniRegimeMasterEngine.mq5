...
input ulong  InpMagicNumber = 26061603;
input int    InpTimerSeconds = 30;
input int    InpReentryCooldownMinutes = 1440;
...
input group "=== Risk ==="
input double InpMaxTotalDrawdownPct = 25.0;
input double InpMaxDailyLossPct = 5.0;
input double InpMaxGlobalRiskPct = 10.0;
...
input int    InpDailyEntryPauseStartHour = 23;
input int    InpDailyEntryPauseStartMinute = 0;
input int    InpDailyEntryPauseEndHour = 0;
input int    InpDailyEntryPauseEndMinute = 5;
...
input string InpPushPrefix = "OmniMaster";
...
bool IsDailyEntryPause(string &reason)
{
   if(InpDailyEntryPauseStartHour < 0 || InpDailyEntryPauseEndHour < 0)
   {
      reason = "daily entry pause disabled";
      return false;
   }
   ...
   int startMinutes = InpDailyEntryPauseStartHour * 60 + InpDailyEntryPauseStartMinute;
   int endMinutes = InpDailyEntryPauseEndHour * 60 + InpDailyEntryPauseEndMinute;
   ...
   bool paused = false;
   if(startMinutes < endMinutes)
      paused = (currentMinutes >= startMinutes && currentMinutes < endMinutes);
   else
      paused = (currentMinutes >= startMinutes || currentMinutes < endMinutes);

   reason = paused ? "daily entry pause" : "outside daily entry pause";
   return paused;
}

bool BuildMasterSignal(const SOmniSymbol &item,
                       const SOmniMarketSnapshot &snapshot,
                       const double riskPct,
                       SOmniSignal &signal)
{
   if(snapshot.regime == OMNI_REGIME_RANGE)
      return g_rangeStrategy.BuildSignal(item, snapshot, riskPct, signal);

   if(snapshot.regime == OMNI_REGIME_TREND_UP ||
      snapshot.regime == OMNI_REGIME_TREND_DOWN)
      return g_trendStrategy.BuildSignal(item, snapshot, riskPct, signal);

   OmniResetSignal(signal);
   signal.product = item.product;
   signal.symbol = item.resolvedSymbol;
   signal.reason = "master skipped: regime=" + OmniRegimeName(snapshot.regime);
   return false;
}
                            ...
                            const SOmniSignal &signal)
{
   ...
   if(!g_entryGuard.AllowInitialEntry(signal.product, snapshot.h1BarTime, reason))
   {
      if(InpVerboseLog)
         g_notify.Info(g_symbols[index].logicalName + " skipped: " + reason);
      return false;
   ...
   if(!g_entryGuard.AllowReentryAfterExit(signal.product, InpReentryCooldownMinutes, reason))
   {
      if(InpVerboseLog)
         g_notify.Info(g_symbols[index].logicalName + " skipped: " + reason);
      return false;
...
void MarkExitCooldownFromDeal(const ulong dealTicket)
{
   ...
   for(int i = 0; i < ArraySize(g_symbols); i++)
   {
      if(g_symbols[i].enabled && g_symbols[i].resolvedSymbol == dealSymbol)
      {
         g_entryGuard.MarkExit(g_symbols[i].product, dealTime);
         return;
      }
   }
}
...
void ScanSignals()
{
   ...
   if(IsDailyEntryPause(entryBlockReason))
   {
      if(InpVerboseLog)
         g_notify.Info("New entries paused: " + entryBlockReason);
      return;
   ...
   for(int i = 0; i < OMNI_PRODUCT_COUNT; i++)
   {
      ...
      BuildMasterSignal(g_symbols[i], snapshots[i], riskPct, signals[i]);
      if(signals[i].type == OMNI_SIGNAL_NONE)
      {
         if(InpVerboseLog)
            g_notify.Info(g_symbols[i].logicalName + ": " + signals[i].reason);
         continue;
      }

      if(!EntryGuardAllowsSignal(i, snapshots[i], signals[i]))
         OmniResetSignal(signals[i]);
   }
...
int OnInit()
{
   ...
   g_notify.Info("Starting OmniRegimeMasterEngine. " + g_account.Summary());
...
void OnDeinit(const int reason)
{
   ...
   for(int i = 0; i < ArraySize(g_market); i++)
      g_market[i].Deinit();
   g_notify.Info("OmniRegimeMasterEngine stopped. reason=" + IntegerToString(reason));
}
                        ...
                        const MqlTradeResult &result)
{
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
      MarkExitCooldownFromDeal(trans.deal);
}
