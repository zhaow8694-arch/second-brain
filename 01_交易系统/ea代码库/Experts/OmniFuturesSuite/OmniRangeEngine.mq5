...
input ulong  InpMagicNumber = 26061601;
...
input string InpPushPrefix = "OmniRange";
...
{
   string reason = "";
   if(g_entryGuard.ShouldBlockRangeNewEntry(InpRangeForceCloseHour,
                                            InpRangeForceCloseMinute,
                                            reason))
   {
      if(InpVerboseLog)
...
int OnInit()
{
   ...
   g_notify.Info("Starting OmniRangeEngine. " + g_account.Summary());
...
void OnDeinit(const int reason)
{
   ...
   for(int i = 0; i < ArraySize(g_market); i++)
      g_market[i].Deinit();
   g_notify.Info("OmniRangeEngine stopped. reason=" + IntegerToString(reason));
}
