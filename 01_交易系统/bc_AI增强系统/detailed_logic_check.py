...
def check_execute_trading_logic_details():
    ...
    if execute_logic:
        ...
        print(f"   📝 ExecuteTradingLogic函数内容:")
...
def check_normal_order_implementation():
    """检查普通仓位的具体实现"""
    print("\n🔍 检查普通仓位的具体实现...")
    ...
    can_place_normal = find_and_show_function('CanPlaceNormalOrder', content)
    
    # 检查普通仓位在ExecuteTradingLogic中的使用
    execute_logic = re.search(r'void\s+ExecuteTradingLogic\s*\([^)]*\)\s*\{[^}]*\}', content, re.DOTALL)
    if execute_logic:
        logic_content = execute_logic.group()
        
        print(f"\n🔍 普通仓位在ExecuteTradingLogic中的使用:")
        
        # 查找普通仓位相关的代码行
        lines = logic_content.split('\n')
        for i, line in enumerate(lines):
            if any(keyword in line for keyword in ['Normal', 'normal', '普通']):
                print(f"      {i+1}: {line.strip()}")
    
    return True

def check_lock_order_implementation():
    """检查锁仓的具体实现"""
    print("\n🔍 检查锁仓的具体实现...")
    ...
    can_place_lock = find_and_show_function('CanPlaceLockOrder', content)
    
    # 检查CLockOrderProtector类
    lock_protector = re.search(r'class\s+CLockOrderProtector\s*\{[^}]*\}', content, re.DOTALL)
    if lock_protector:
        print(f"\n🔍 找到CLockOrderProtector类:")
        class_content = lock_protector.group()
    ...
    # 检查锁仓在ExecuteTradingLogic中的使用
    execute_logic = re.search(r'void\s+ExecuteTradingLogic\s*\([^)]*\)\s*\{[^}]*\}', content, re.DOTALL)
    if execute_logic:
        logic_content = execute_logic.group()
        
        print(f"\n🔍 锁仓在ExecuteTradingLogic中的使用:")
        
        # 查找锁仓相关的代码行
        lines = logic_content.split('\n')
        for i, line in enumerate(lines):
            if any(keyword in line for keyword in ['Lock', 'lock', '锁仓']):
                print(f"      {i+1}: {line.strip()}")
...
def main():
    ...
    # 检查普通仓位实现
    check_normal_order_implementation()
    
    # 检查锁仓实现
    check_lock_order_implementation()
