...
input double DecisionReversalThreshold = 0.55;    // 决策反转概率阈值（概率制）
...
int OnInit()
{
    ...
    Print("  ⏰ 建仓频率: 15分钟间隔控制");
    Print("📊 MACD算法优化:");
    Print("  🔧 强度计算: 降低激进程度，避免频繁满强度");
    Print("  🎯 质量评估: 零轴位置、信号距离、柱状图幅度多维度评估");
    Print("  ⚖️ 权重控制: 单个指标贡献不超过总强度40%");
    Print("  📈 信号分级: 明确信号100%信心，轻微信号40%信心");
    Print("  🛡️ 噪音过滤: 增加最小柱状图幅度要求");
    Print("🔧 核心功能:");
