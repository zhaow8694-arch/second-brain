...
void CalculateDecisionScores(double &buy_score, double &sell_score)
{
    ...
    // 3. MACD动量信号 (技术权重25%) - 买入vs卖出强度分配
    if(macd_main != 0 && macd_signal_line != 0) {
        // MACD强度基于柱状图和主线位置
        double histogram_strength = MathMin(MathAbs(macd_histogram) * 5000, 1.0); // 0.0002为满强度
        double main_strength = MathMin(MathAbs(macd_main) * 2000, 1.0); // 0.0005为满强度
        double macd_strength = (histogram_strength + main_strength) / 2.0; // 综合强度
        double macd_weight = base_technical_weight * 0.25;
        
        // MACD买入/卖出信号分配
        if(macd_main > macd_signal_line && macd_histogram > 0) {
            // 明确买入信号：主线上穿且柱状图为正
            double macd_buy_contribution = macd_strength * macd_weight;
            ...
            // 轻微买入倾向：仅柱状图为正
            double macd_buy_contribution = macd_strength * macd_weight * 0.5; // 50%强度
            total_buy_strength += macd_buy_contribution;
            Print("📊 MACD信号: 轻微买入 柱状图=", DoubleToString(macd_histogram, 5),
                  " 强度=", DoubleToString(macd_strength * 0.5, 3),
                  " 买入贡献=", DoubleToString(macd_buy_contribution, 4));
                  
        } else if(macd_histogram < 0) {
            // 轻微卖出倾向：仅柱状图为负
            double macd_sell_contribution = macd_strength * macd_weight * 0.5; // 50%强度
            total_sell_strength += macd_sell_contribution;
            Print("📊 MACD信号: 轻微卖出 柱状图=", DoubleToString(macd_histogram, 5),
                  " 强度=", DoubleToString(macd_strength * 0.5, 3),
                  " 卖出贡献=", DoubleToString(macd_sell_contribution, 4));
        } else {
            Print("📊 MACD信号: 中性 主线=", DoubleToString(macd_main, 5), 
                  " 柱状图=", DoubleToString(macd_histogram, 5));
        }
