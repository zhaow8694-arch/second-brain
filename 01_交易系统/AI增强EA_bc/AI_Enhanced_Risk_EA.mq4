...
input double DecisionReversalThreshold = 0.6;     // 决策反转评分阈值
...
public:
    ...
    void UpdateIndicators()
    {
        ...
        if(current_time != last_update_time || tick_count >= 10)
        {
        ...
        for(int i = 0; i < OrdersTotal(); i++)
        {
            if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
            {
                if(OrderSymbol() == Symbol() && OrderMagicNumber() == 12345)
                {
                    ...
                    {
                        double profit = ::GetCurrentOrderTotalProfit();
                        double profit_pips = profit / Point;
                        int ticket = OrderTicket();
...
public:
    ...
    // 计算前5个决策评分的主要方向 (简化版，避免Stack overflow)
    int CalculateHistoricalDirection()
    {
        // 简化历史方向计算
        if(g_decision_history_count < 3) return 0;
        
        // 只检查最近的2个记录
        int buy_count = 0;
        int sell_count = 0;
        
        for(int i = 1; i < MathMin(3, g_decision_history_count); i++)
        {
        ...
        if(buy_count > sell_count) return 1;
        else if(sell_count > buy_count) return -1;
        else return 0;
    }
    ...
    void ExecuteOrder(int order_type)
    {
        ...
        if(order_type == OP_BUY)
        {
            int ticket = OrderSend(Symbol(), OP_BUY, lot_size, Ask, (int)MaxSlippage, 0.0, 0.0, 
                                 "AI_Enhanced", 12345, 0.0, clrGreen);
...
int OnInit()
{
    ...
    Print("同方向最大持仓: ", MaxSameDirectionOrders, " (支持14个总持仓)");
    Print("=== 手数设置 ===");
