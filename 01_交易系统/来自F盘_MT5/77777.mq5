...
input int    InpMagicNumber   = 208300;
...
bool SendTelegram(string message)
{
   ...
   if(res!=200) Print("❌ Telegram推送失败 Code:",res);
   return (res==200);
}
...
int OnInit()
{
   ...
   string msg = "🚀 <b>Guardian Earth V20.8.3 已启动</b>\n品种: " + _Symbol + "\n风险: " + DoubleToString(InpRiskPercent,1) + "%\n时间: " + TimeToString(TimeCurrent(),TIME_DATE|TIME_MINUTES);
   SendTelegram(msg);

   Print("🚀 V20.8.3 纯净编译版启动 | 初始基数: ", DailyStartBalance);
...
bool IsSecurityBreached()
{
   ...
   // 实时浮动持仓中去除了已被官方弃用的 POSITION_COMMISSION
   for(int i=PositionsTotal()-1;i>=0;i--)
      if(PositionGetSymbol(i)==_Symbol && PositionGetInteger(POSITION_MAGIC)==InpMagicNumber) 
...
void OnTick()
{
   if(IsSecurityBreached()){Comment("🌍 V20.8.3 | 熔断或周末休市中"); return;}
   ...
   for(int i=PositionsTotal()-1;i>=0;i--){
      if(PositionGetSymbol(i)==_Symbol && PositionGetInteger(POSITION_MAGIC)==InpMagicNumber){
         ...
         // 去除已弃用的 POSITION_COMMISSION，使用 PROFIT + SWAP
