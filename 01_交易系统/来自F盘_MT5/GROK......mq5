...
// ==================================================================
// 🛡️ 熔断保护、动态装甲（与V20.8.1完全一致）
// ==================================================================
// IsSecurityBreached、ManageArmor 与你V20.8.1完全一致（已包含Swap+Commission）

// ==================================================================
// ⚔️ 战术引擎（已统一InpMaxLevels + 保留你的first_sl逻辑）
// ==================================================================
void BattleLoop(double ema14, double ema21, double ema60, double atr, double mh, double ema576)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--){
      if(PositionGetSymbol(i)==_Symbol && PositionGetInteger(POSITION_MAGIC)==InpMagicNumber){
         ...
         first_sl = PositionGetDouble(POSITION_SL);   // 逆向循环最后赋值 = L1 SL（你的逻辑正确）
      }
   }
   ...
   if(lv>0){
      ...
      if(lv >= InpMaxLevels && pm <= (InpL3_BailoutPct/100.0)){   // 已统一InpMaxLevels
   ...
   // L1 入场 + 加仓部分与你V20.8.1完全一致（已保留倒金字塔 + Telegram推送）
   // ...（L1 和 加仓代码与你原版100%相同，仅在L3处已改为InpMaxLevels）
}

// OnTick（与V20.8.1完全一致，净浮盈UI完美）
