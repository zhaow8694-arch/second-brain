...
input double InpRiskBasePct          = 0.8;      // default risk per trade (%)
input double InpMaxDailyDrawdownPct  = 10.0;     // daily drawdown cutoff (%)
input int    InpMaxConcurrentPos     = 2;        // global max open positions
...
input int    InpTradeDirection       = 0;        // 0 both, 1 long only, -1 short only
...
bool ExecuteTradeResult(const bool ok, const string action, const string symbol, const ulong ticket){
   ...
   if(rc == TRADE_RETCODE_NO_MONEY || rc == TRADE_RETCODE_INVALID_VOLUME || rc == TRADE_RETCODE_INVALID_PRICE){
      ...
      Print("GE margin-related fail detected, suspend new entries for ",
            InpMarginFailCooldownMin, " min");
   }
   return false;
}
...
bool StopLossIsTradable(const string symbol, const long type, const double stopLoss){
   ...
   double minDist = MathMax(1, stopsLevel) * point;
...
int DetectSignal(datetime &signalBarTime){
   ...
   double e14[3], e21[3], e60[3], atr[3], macdMain[3], macdSig[3], hist[3], e4h[2];
   ...
   double v1 = iVolume(_Symbol, PERIOD_M15, 1);
   double v0 = iVolume(_Symbol, PERIOD_M15, 2);
   bool volOk = true;
...
int OnInit(){
   ...
   Print("Guardian Earth loaded on ", _Symbol);
...
void OnTick(){
    ...
    for(int i=0;i<ArraySize(g_symbols);i++) EnsureSymbolState(g_symbols[i]);
