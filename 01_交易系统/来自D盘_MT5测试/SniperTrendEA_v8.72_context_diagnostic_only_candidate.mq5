...
input string InpComment = "SniperEA_v8.72_context_diag";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.72_context_diagnostic_only_candidate";
...
int OnInit()
{
   ...
   if(InpUseV869RegimeGate || InpV869EntryDiagnosticLog || InpUseV869DiEdgeHourGate || InpUseV869DiEdgeHourScale || InpUseV869ResidualHourScale || InpUseV869ContextScale || InpUseV871RealStrongContext || InpUseV872ContextDiagnostics)
   {
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyUp))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if(!ApplySpreadCostGovernor(lot, strong_context) || !ApplyV871ContextGate(lot, true))
                     {
                        g_pendingBuy = false; g_pendingBars = 0;
                     }
                     else
               ...
               adxOk && timeOk && atrFilterOk && (!InpUseDailyFilter || dailyDown))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     if(!ApplySpreadCostGovernor(lot, strong_context) || !ApplyV871ContextGate(lot, false))
                     {
                        g_pendingSell = false; g_pendingBars = 0;
                     }
                     else
