...
input double InpRegimeWatchThreshold     = 92.0;
input double InpRegimeRiskThreshold      = 82.0;
input int    InpRegimeMinLossesForScale  = 2;
...
input string InpComment = "SniperEA_v8.69_fix3b";
...
const string EA_VERSION_NAME      = "SniperTrendEA_v8.69_fix3b_loss_armed_threshold_gate";
...
double GetV869IndicatorValue(int handle, int bufferIndex, int shift)
{
   ...
   if(copied <= 0)
   {
      if(InpRegimeDebugLog)
         Print("[v8.69_fix3b RegimeGate] CopyBuffer failed | handle=", handle,
...
double GetV869RegimeScore()
{
   ...
   if(InpRegimeDebugLog)
      Print("[v8.69_fix3b RegimeGate] score=", DoubleToString(score, 1),
...
double GetV869RegimeRiskMultiplier()
{
   ...
   int lossesForScale = GetConsecutiveLosses();

   if(lossesForScale >= InpRegimeMinLossesForScale)
   {
      if(score <= InpRegimeHardBlockThreshold)
         multiplier = 0.0;
      else if(score < InpRegimeRiskThreshold)
         multiplier = InpRegimeRiskScaleRisk;
      else if(score < InpRegimeWatchThreshold)
         multiplier = InpRegimeRiskScaleWatch;
   }

   if(InpRegimeDebugLog)
      Print("[v8.69_fix3b RegimeGate] multiplier=", DoubleToString(multiplier, 2),
            " score=", DoubleToString(score, 1),
            " hardBlock=", DoubleToString(InpRegimeHardBlockThreshold, 1),
            " riskThreshold=", DoubleToString(InpRegimeRiskThreshold, 1),
            " watchThreshold=", DoubleToString(InpRegimeWatchThreshold, 1),
            " lossesForScale=", lossesForScale,
            " minLosses=", InpRegimeMinLossesForScale);
...
double AdjustLotSize(double lot)
{
   ...
   {
      if(InpRegimeDebugLog)
         Print("[v8.69_fix3b RegimeGate] new entry blocked by regime multiplier");
      return 0.0;
   }
