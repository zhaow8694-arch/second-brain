...
public:
   ...
   void Init(CTradeWrapper* trade, CLogger* logger, string symbol,
             ...
             int maxTrend, int maxRange, int maxTotal)
   {
      m_trade             = trade;
      m_logger            = logger;
      m_symbol            = symbol;
   ...
   void ManagePositionHandover(ENUM_MARKET_STATE oldState, ENUM_MARKET_STATE newState)
   {
      ...
      {
         if(m_logger) m_logger.Info("State handover: TREND->RANGE - Tightening stops on existing trend positions");
         ...
         for(int i = 0; i < total; i++)
         {
            ...
            string comment = PositionGetString(POSITION_COMMENT);
            if(StringFind(comment, "TREND") < 0) continue;
            ...
            if(CopyBuffer(atrHandle, 0, 0, 1, atr) <= 0) { IndicatorRelease(atrHandle); continue; }
            IndicatorRelease(atrHandle);

            double tightSLDist = atr[0] * 1.0;
            double newSL, newTP;
            newTP = currentTP;
            ...
            if(posType == POSITION_TYPE_BUY)
            {
               ...
               if(profit > 0) newSL = NormalizeDouble(MathMax(newSL, openPrice + 5 * SymbolInfoDouble(m_symbol, SYMBOL_POINT)), digits);
            }
            else
            {
               double ask = SymbolInfoDouble(m_symbol, SYMBOL_ASK);
               newSL = NormalizeDouble(ask + tightSLDist, digits);
               if(profit > 0) newSL = NormalizeDouble(MathMin(newSL, openPrice - 5 * SymbolInfoDouble(m_symbol, SYMBOL_POINT)), digits);
            }

            if(m_trade.ModifySLTP(ticket, newSL, newTP))
            {
               if(m_logger) m_logger.Info("Handover: Tightened SL on legacy position #" + IntegerToString(ticket));
            }
         }
      }
   }
   ...
   void ManageTrailingStops(bool isTrendMode)
   {
      ...
      for(int i = 0; i < total; i++)
      {
         ...
         {
            trailStart = atr[0] * 2.0;
            trailStep  = atr[0] * 0.5;
         }
         else
         {
            trailStart = atr[0] * 0.8;
            trailStep  = atr[0] * 0.3;
         }
