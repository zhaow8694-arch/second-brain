//+------------------------------------------------------------------+
//|                                  SniperTrendEA_v8.7_RiskFix.mq5 |
//|     基于威科夫趋势线突破 + Evil MACD + Z-Wei 哲学过滤的优化版本      |
//|                                                                  |
//|  v8.7 优化重点：                                                  |
//|  1) 保留 v8.6 核心逻辑：MACD 翻转、MA200趋势、危险K线、影线过滤。 |
...
//|  7) 【关键修复】手数按 OrderCalcProfit 预估 1手止损亏损，避免XAUUSD等品种风险被低估。 |
//|  8) 增加最小止损推进阈值，减少保本/移动止损的细碎修改请求。                               |
//+------------------------------------------------------------------+
#property copyright "SniperTrendEA v8.7 - Risk Fixed by ChatGPT"
...
input bool   InpUseOrderCalcProfitRisk = true;  // 【v8.7】用OrderCalcProfit按账户货币精确估算止损亏损
input bool   InpAllowTickValueFallback = false; // 【v8.7】OrderCalcProfit失败时是否允许TickValue兜底（默认关闭更安全）
input bool   InpPrintRiskDetails = true;    // 【v8.7】开仓前打印计划风险金额/风险百分比
...
input double InpMinStopImproveATR = 0.05;   // 【v8.7】止损至少改善多少ATR才修改，降低频繁改SL
input int    InpMinStopImprovePoints = 10;  // 【v8.7】止损至少改善多少points才修改

input group "=== 交易执行设置 ==="
input int    InpMagicNumber      = 20260506;
input string InpComment          = "SniperEA_v8.7_riskfix";
...
int OnInit()
{
   ...
   Print("SniperTrendEA v8.7 RiskFix 初始化成功 | ", _Symbol, " ", EnumToString((ENUM_TIMEFRAMES)_Period),
         ...
         " | 风险计算:", InpUseOrderCalcProfitRisk ? "OrderCalcProfit" : "TickValue",
         " | 最小改SL:", InpMinStopImproveATR, "×ATR/", InpMinStopImprovePoints, "pt");
...
bool ValidateInputs()
{
   ...
   if(!InpUseOrderCalcProfitRisk && !InpAllowTickValueFallback)
   {
      Print("参数错误：关闭OrderCalcProfit风险计算时，必须允许TickValue兜底，否则无法计算手数");
      return false;
   }
   ...
   if(InpMinStopImproveATR < 0.0 || InpMinStopImprovePoints < 0)
   {
      Print("参数错误：最小止损推进阈值不能为负");
      return false;
   }
                ...
                bool adxOk, bool timeOk, bool atrFilterOk, bool dailyUp, bool spreadOk)
{
   ...
   Print("【开多】实体:", DoubleToString(c.bodyRatio * 100.0, 1), "%",
         ...
         InpPrintRiskDetails ? StringFormat(" | Risk:%.2f (%.2f%%)",
                                            EstimatePlannedLoss(ORDER_TYPE_BUY, price, sl, lot),
                                            EstimatePlannedLoss(ORDER_TYPE_BUY, price, sl, lot) /
                                            MathMax(1.0, (InpUseEquityRisk ? AccountInfoDouble(ACCOUNT_EQUITY) : AccountInfoDouble(ACCOUNT_BALANCE))) * 100.0) : "");
                 ...
                 bool adxOk, bool timeOk, bool atrFilterOk, bool dailyDown, bool spreadOk)
{
   ...
   Print("【开空】实体:", DoubleToString(c.bodyRatio * 100.0, 1), "%",
         ...
         " | Lot:", DoubleToString(lot, VolumeDigits()),
         InpPrintRiskDetails ? StringFormat(" | Risk:%.2f (%.2f%%)",
                                            EstimatePlannedLoss(ORDER_TYPE_SELL, price, sl, lot),
                                            EstimatePlannedLoss(ORDER_TYPE_SELL, price, sl, lot) /
                                            MathMax(1.0, (InpUseEquityRisk ? AccountInfoDouble(ACCOUNT_EQUITY) : AccountInfoDouble(ACCOUNT_BALANCE))) * 100.0) : "");
...
bool PrepareTradePlan(ENUM_ORDER_TYPE type, double atr, double &price, double &sl, double &lot, string &reason)
{
   ...
   lot = CalculateLotSize(type, price, sl);
   if(lot <= 0.0)
   {
      reason = "按当前风险计算出的手数无效或低于最小手数，或无法可靠估算止损亏损";
      return false;
   }
...
//+------------------------------------------------------------------+
//| 手数计算（v8.7：优先使用 OrderCalcProfit，避免XAUUSD等CFD风险错算） |
//+------------------------------------------------------------------+
double CalculateLotSize(ENUM_ORDER_TYPE type, double entryPrice, double slPrice)
{
   if(entryPrice <= 0.0 || slPrice <= 0.0 || MathAbs(entryPrice - slPrice) <= 0.0)
      return 0.0;
   ...
   if(InpUseOrderCalcProfitRisk)
   {
      double oneLotProfit = 0.0;
      ResetLastError();
      bool calcOk = OrderCalcProfit(type, _Symbol, 1.0, entryPrice, slPrice, oneLotProfit);
      if(calcOk)
         lossPerLot = MathAbs(oneLotProfit);
      else
      {
         int err = GetLastError();
         Print("【风险计算警告】OrderCalcProfit失败 | 错误:", err,
               " | Type:", EnumToString(type),
               " | Entry:", DoubleToString(entryPrice, _Digits),
               " | SL:", DoubleToString(slPrice, _Digits));
      }

      if(lossPerLot <= 0.0 && !InpAllowTickValueFallback)
         return 0.0;
   }
   ...
   if(lossPerLot <= 0.0 && InpAllowTickValueFallback)
   {
      ...
      lossPerLot = CalculateLossPerLotByTickValue(slDist);
      if(lossPerLot <= 0.0)
         return 0.0;

      Print("【风险计算警告】使用TickValue兜底计算手数，请确认该品种TickValue是否可靠 | lossPerLot:",
...
double CalculateLossPerLotByTickValue(double slDist)
{
   if(slDist <= 0.0)
      return 0.0;
...
double EstimatePlannedLoss(ENUM_ORDER_TYPE type, double entryPrice, double slPrice, double lot)
{
   if(lot <= 0.0 || entryPrice <= 0.0 || slPrice <= 0.0)
      return 0.0;

   double profit = 0.0;
   ResetLastError();
   if(OrderCalcProfit(type, _Symbol, lot, entryPrice, slPrice, profit))
      return MathAbs(profit);

   if(InpAllowTickValueFallback)
      return CalculateLossPerLotByTickValue(MathAbs(entryPrice - slPrice)) * lot;
...
double MinStopImproveDistance(double atr)
{
   double byAtr = (atr > 0.0 ? atr * InpMinStopImproveATR : 0.0);
   double byPoint = (double)InpMinStopImprovePoints * _Point;
   double minImprove = MathMax(byAtr, byPoint);

   if(minImprove < _Point)
      minImprove = _Point;
...
void ManageProtectionStops(double atr)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(posType == POSITION_TYPE_BUY)
      {
         ...
         double minImprove = MinStopImproveDistance(atr);
         if(hasCandidate && (curSL == 0.0 || candidateSL > curSL + minImprove))
