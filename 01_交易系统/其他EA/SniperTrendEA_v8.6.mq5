...
//|                    v8.6 - ATR动态止盈版（最优解）                  |
//|                                                                  |
//|  v8.6 在 v8.5 方案A基础上仅加一项——开仓即设止盈 9.0×ATR：          |
//|                                                                  |
//|  ✦ 价格走到 9×ATR 后自动止盈落袋                                  |
//|  ✦ TrailStart=5.0ATR 的移动止盈先启动                            |
//|  ✦ TP=9.0 只在 Trail 都兜不住的极端行情才触发                     |
//|                                                                  |
//|  十年全期验证（2015-2025, H4, XAUUSD）：                           |
//|    PF 2.41（↑）  |  回撤 16.04%（原27%↓41%）                       |
//|    夏普 4.64（↑） |  采收 5.17（↑）                                |
//|                                                                  |
//|  入场条件：与v8.5方案A完全相同，一像素未改。                        |
//|  移动止盈：完全保留。盈亏平衡：已证伪，默认关闭。                    |
//+------------------------------------------------------------------+
#property copyright "SniperTrendEA v8.6 - TP 9.0ATR Optimal"
